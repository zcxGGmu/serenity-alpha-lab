...
- [x] Confirm actual git state with `git status --short --branch` and `git log -8 --oneline`.
- [x] Update `docs/development-status.md` to show completed through `SAL-P6-003`, unfinished from `SAL-P6-004`, current checkpoints and recovery prompt.
- [x] Update `docs/development-progress-checklist.md` tail summary with the latest `SAL-P6-003` anchors.
...
- Completed range is `SAL-P0-001..013`, `SAL-P1-001..016`, `SAL-P2-001..020`, `SAL-P3-001..017`, `SAL-P4-001..022`, `SAL-P5-001..018`, `SAL-P6-001..003`; unfinished range starts at `SAL-P6-004`.
- Latest implementation checkpoint entering this review is `984813fd feat(P6): 加固密钥与配置`; latest status-sync checkpoint is `00418b3d docs: 同步 SAL-P6-003 checkpoint hash`; latest status-sync hash record is `70b003c2 docs: 记录 SAL-P6-003 状态同步 hash`.
- Status-review checkpoint created: `dc55fad0 docs: 复核 SAL-P6-003 最新开发状态与恢复提示`; hash-record checkpoint created: `3574678e docs: 记录 SAL-P6-003 状态复核 hash`.

---

# SAL-P6-003 Secret And Config Hardening Implementation Plan

> Scope: Complete only `SAL-P6-003` by adding framework-neutral Secret Manager/OS Keychain reference contracts, rotation simulation, safe config diagnostics and config audit records. Do not start `SAL-P6-004+`, SSRF/file upload hardening, SCA gates, OpenTelemetry, backup/restore, real Provider/LLM, Worker loop, Qlib runtime, production scheduling, notification sender, release packaging or formal backtest promotion.

## Checklist

- [x] Re-read required recovery, progress, security, Auth/RBAC, Resource/Artifact authorization, upstream patch and config/profile docs.
- [x] Confirm actual git state with `git status --short --branch` and `git log -8 --oneline`.
- [x] Write SAL-P6-003 implementation plan in `docs/superpowers/plans/2026-07-31-secret-config-hardening.md`.
- [x] Add failing contract tests for Secret Manager/OS Keychain references, config API safe diagnostics, rotation simulation and config audit redaction.
- [x] Run focused Red targets and record expected failures.
- [x] Extend framework-neutral `application.config_profiles` with metadata-only secret reference, safe diagnostic, rotation and audit contracts.
- [x] Export public secret/config hardening symbols and add an architecture import guard proving no keychain/cloud SDK/FastAPI/Provider/LLM/Worker/Qlib runtime imports.
- [x] Add `docs/secret-config-hardening.md` evidence record and update progress/status registers for P6 `3/23`, total `109/129`, `SAL-P6-004` next.
- [x] Run focused Green, related Auth/RBAC + ResourceAuthorization + architecture suite, full pytest, compileall, dependency lock, immutable tag and diff hygiene checks.
- [x] Create Chinese checkpoint commit for `SAL-P6-003`.

## Recovery Anchors

- Phase: P6 安全、稳定性与发布加固.
- Gate: G5 已通过；G6 未通过.
- Completed entering task: `SAL-P0-001..013`, `SAL-P1-001..016`, `SAL-P2-001..020`, `SAL-P3-001..017`, `SAL-P4-001..022`, `SAL-P5-001..018`, `SAL-P6-001..002`.
- Current READY task: `SAL-P6-003` 加固密钥与配置.
- Implementation checkpoint entering task: `33f76bad feat(P6): 实现资源与 Artifact 授权`.
- Latest final anchor entering task: `b4de1769 docs: 固化 SAL-P6-002 状态复核 hash-anchor`.
- Strict boundary: do not start `SAL-P6-004+`, real Provider/LLM, Worker loop, Qlib runtime, production scheduling, notification sender, release packaging or formal backtest promotion.

## Review

- Plan check-in completed before code changes. Implementation stayed application-layer only and runtime-free.
- Red target failed as expected during collection with missing `ConfigAuditAction` import from `serenity_alpha_lab.application.config_profiles` (`1 error`).
...
- Status-review checkpoint is `981c4b4f docs: 复核 SAL-P6-002 最新开发状态与恢复提示`; status-review hash record checkpoint is `3f97b071 docs: 记录 SAL-P6-002 状态复核 hash`.
