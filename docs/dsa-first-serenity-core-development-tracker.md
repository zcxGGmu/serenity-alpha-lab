...
- Serenity 最新提交：请以下次启动时 `git rev-parse --short HEAD` 为准；当前 tracker 将刷新到 P4-T03 完成后的状态
...
- P4-T01 已新增 DSA `src/serenity/core/provenance.py`
- P4-T01 已定义稳定 provenance id：`serenity:<analysis_id>:<evidence_id>`
- P4-T01 已定义 internal `dsa://analysis/<analysis_id>/evidence/<evidence_id>` source URI
- P4-T01 已把 `provenance` block 附加到 `serenity_research`
- P4-T01 已为 coverage summary、coverage flags、acquisition tasks 和 persisted research tasks 附加 sanitized `source_refs`
- P4-T01 source ref 只保留 title、publisher、external URL、raw source URL、published_at、excerpt hash、claim id、missing reason 等可审计低敏字段
- P4-T01 会把缺少 traceable source 的 evidence 标记为 explicit missing provenance，并在 provenance diagnostics 中列出 missing provenance ids
- P4-T01 已更新 API schema、storage sanitizer 和 Intelligence task sanitizer，使 provenance refs 可通过 snapshot-first 流程保留
- P4-T01 未新增 DB 表、migration、provider fetch、Agent prompt、交易建议字段映射或 DSA trading field 语义变更
- P4-T02 已新增 DSA `src/serenity/core/report_safety.py`
- P4-T02 已新增 DSA `tests/serenity/core/test_report_safety.py`
- P4-T02 已更新 DSA `docs/serenity-integration-boundaries.md`，记录 scanner scope、severity、quoted-source exception 和 runtime 边界
- P4-T02 scanner 只扫描 Serenity-generated research-only sections、显式传入 section、tool output / diagnostics / task copy 等研究质量文案
- P4-T02 scanner 默认不扫描、不阻断、不改写 DSA 主报告字段；`dsa_main_report` 与普通 top-level `coverage` / `readiness` / `tasks` payload 保持 out-of-scope
- P4-T02 会把 Serenity-generated 直接买入/卖出/持有/加仓/减仓指令、目标价、仓位建议、保证收益、确定性预测标记为 error
- P4-T02 会把 Serenity payload 中意外出现的 DSA 交易字段（如 `operation_advice`）标记为 error
- P4-T02 quoted source excerpt 只有在显式标记并带 sanitized `provenance_id` 时才作为 info 豁免；缺少 provenance 会返回 warning
- P4-T02 missing source refs for generated claim 会返回 warning，用于 release checklist / CI gate 后续接入
- P4-T02 未新增 DB 表、migration、provider fetch、Agent prompt、UI page、通知变更、交易建议字段映射或 DSA trading field 语义变更
- P4-T03 已新增 DSA `scripts/serenity_release_check.py`
- P4-T03 release check 输出 machine-readable JSON，包含 `schema_version`、`status`、`error_count`、`warning_count`、checks 和 rollback 信息
- P4-T03 release check 运行五类离线 gate：`flag_off_baseline`、`flag_on_research_audit`、`serenity_unit_tests`、`report_safety_scanner`、`serenity_py_compile`
- P4-T03 release check 不需要 secrets、外部行情数据、broker token、provider credentials、通知配置或 DB migration
- P4-T03 已新增 DSA `docs/RELEASE_CHECKLIST.md`，记录 flag-off / flag-on 验证、failure handling、`SERENITY_RESEARCH_ENABLED=false` rollback 与 `git revert` 指引
- P4-T03 已把 release check 接入 DSA PR CI 的 backend gate
...
P4-T01 验证证据：
- red check `python3.11 -m pytest tests/serenity/core/test_provenance.py tests/serenity/services/test_evidence_quality_service.py::test_evidence_quality_service_enabled_outputs_research_only_audit tests/serenity/services/test_evidence_quality_service.py::test_evidence_quality_service_empty_context_returns_stable_blocked_audit tests/serenity/services/test_research_task_service.py::test_build_tasks_from_audit_generates_schema_valid_stable_ids tests/serenity/test_serenity_api_schema_contract.py::test_analysis_result_response_accepts_optional_serenity_research_audit -q` -> collection error，缺少 `src.serenity.core.provenance`
- after fix same focused command -> `7 passed`
- focused P4/API/snapshot/intelligence regression `python3.11 -m pytest tests/serenity/core/test_provenance.py tests/serenity/services/test_evidence_quality_service.py tests/serenity/services/test_research_task_service.py tests/serenity/test_serenity_api_schema_contract.py tests/serenity/test_analysis_service_serenity.py tests/api/v1/test_serenity_research_task_schema.py tests/serenity/test_analysis_history_serenity_snapshot.py tests/storage/test_serenity_research_tasks_snapshot.py tests/test_intelligence_api.py -q` -> `54 passed, 1 warning`
- Serenity regression `python3.11 -m pytest tests/serenity -q` -> `42 passed`
- intelligence/API/storage regression `python3.11 -m pytest tests/test_intelligence_api.py tests/api/v1/test_serenity_research_task_schema.py tests/storage/test_serenity_research_tasks_snapshot.py -q` -> `25 passed, 1 warning`
- boundary guard `python3.11 -m pytest tests/test_serenity_integration_boundaries.py -q` -> `3 passed`
- target `py_compile` -> pass
- `git diff --check` -> pass
- safety scan found no forbidden direct trading mappings in P4 diff
- DB-table scan matched existing function/test names plus contract/decision docs only; no migration/table added
- provenance local-path scan -> no matches

P4-T02 验证证据：
- red check `python3.11 -m pytest tests/serenity/core/test_report_safety.py -q` -> collection error，缺少 `src.serenity.core.report_safety`
- after initial implementation focused scanner check -> `8 passed`
- reviewer found two important issues：nested quoted source excerpt 被静默跳过；top-level generic `coverage` / `readiness` / `tasks` scope 过宽
- after review fix focused scanner check -> `12 passed`
- P4 regression `python3.11 -m pytest tests/serenity/core/test_report_safety.py tests/serenity/core/test_provenance.py tests/serenity/test_serenity_api_schema_contract.py tests/serenity/services/test_evidence_quality_service.py tests/serenity/services/test_research_task_service.py -q` -> `33 passed`
- Serenity regression `python3.11 -m pytest tests/serenity -q` -> `54 passed`
- boundary guard `python3.11 -m pytest tests/test_serenity_integration_boundaries.py -q` -> `3 passed`
- target `py_compile` -> pass
- safety scan only matched existing docs/baseline scan command and intentional scanner test assertion
- DB-table scan matched existing snapshot API/function/test names plus contract/decision docs only; no migration/table added
- local-path scan -> no matches
- `git diff --check` -> pass
- independent re-review -> Ready to merge: Yes; no Critical / Important / Minor issues after fixes
...
- [x] DSA 报告生成路径和引用字段已确认。
- [ ] 安全扫描词表、允许引用例外和误报处理策略已确认。

**Phase Exit Criteria:**

- [x] 报告中的 Serenity block 有 provenance namespace。
- [ ] Safety scanner 能区分产品生成文案与 quoted source excerpt。
- [ ] 发布前验证包含 report safety scan。
- [x] 用户可查看 evidence audit trail 或 diagnostics。
...
PR:
Commit: DSA `c7e7c58`
Evidence: Red check failed on missing `src.serenity.core.provenance`; after implementation focused P4 check -> `7 passed`; focused P4/API/snapshot/intelligence regression -> `54 passed, 1 warning`; `tests/serenity -q` -> `42 passed`; intelligence/API/storage regression -> `25 passed, 1 warning`; boundary guard -> `3 passed`; target `py_compile`, `git diff --check`, safety scan, DB-table scan, and provenance local-path scan passed.
Decision Notes: Keep provenance snapshot-first inside `analysis_history.context_snapshot.serenity_research`; do not create DB tables or map source refs into DSA trading semantics.
Rollback Notes: Revert DSA commit `c7e7c58` or remove the `provenance` / `source_refs` API fields and stop passing `analysis_id` into `EvidenceQualityService.evaluate()`.
...
- Modify: `/Users/zq/Desktop/ai-projs/trading/daily_stock_analysis/api/v1/schemas/serenity.py`
...
- [x] 定义 provenance id 格式：`serenity:<analysis_id>:<evidence_id>` 或同等稳定方案。
- [x] 为 audit summary、coverage、gap、task 添加 source refs。
- [x] source ref 包含 title、publisher、url、published_at、excerpt hash 或 claim id。
- [x] 不存完整敏感上下文。
- [x] 对缺 source 的 evidence 显示 explicit missing provenance。
...
- [x] provenance id 稳定且不泄露本地绝对路径。
- [x] 缺引用不是静默通过，而是形成 diagnostics 或 gap。
...
Branch: codex/serenity-phase-0-evidence-bridge
PR:
Commit: DSA `41a8878` (`feat(serenity): 增加报告安全扫描器`)
Evidence: red scanner test failed on missing `src.serenity.core.report_safety`; focused scanner -> `12 passed`; P4 regression -> `33 passed`; Serenity regression -> `54 passed`; boundary guard -> `3 passed`; target `py_compile`, safety scan, DB-table scan, local-path scan, and `git diff --check` passed.
Decision Notes: Scanner remains a deterministic pure-core helper and is not wired into the DSA main report path. It scans only Serenity-generated research-only sections or explicit scan sections, requires provenance for quoted-source exemptions, and treats DSA trading fields inside Serenity payloads as errors.
Rollback Notes: Disable or remove scanner gate while preserving fail-open DSA analysis behavior; deleting `src/serenity/core/report_safety.py` and its tests reverts P4-T02 without DB or API migration.
...
- [x] 定义 forbidden generated phrases：直接买入/卖出/持有命令、目标价承诺、仓位建议、保证收益、确定性预测。
- [x] 定义 allowed quoted source excerpt 区域，避免误杀外部来源引用。
- [x] 扫描 Serenity-generated copy、tool output、UI panel copy。
- [x] 输出 severity、location、matched phrase、remediation。
- [x] 默认只拦截 Serenity report section 发布，不拦截 DSA 主链路。
...
- [x] forbidden generated phrase 被拦截。
- [x] quoted source excerpt 的合法引用不误报。
- [x] scanner 输出可用于 CI 或 release checklist。
...
Branch: `codex/serenity-phase-0-evidence-bridge`
PR:
Commit: DSA `d3127c5` (`feat(serenity): 接入发布检查清单`)
Evidence: red release-check tests failed on missing `scripts/serenity_release_check.py` / `docs/RELEASE_CHECKLIST.md`; tag-release workflow red check failed because release workflows did not run the Serenity gate; focused release-check tests -> `6 passed`; actual `python3.11 scripts/serenity_release_check.py` -> `status passed, errors 0, warnings 0`; actual plan output succeeded; P4 focused regression -> `21 passed`; broader Serenity/Agent/boundary regression -> `85 passed`; `py_compile`, `git diff --check`, local-path scan passed; boundary scan reviewed only existing non-Serenity Docker smoke imports plus release checklist wording.
Decision Notes: Release gate is offline and deterministic, emits machine-readable JSON, runs flag-off/flag-on checks, Serenity unit tests, report safety scanner, and Serenity compile check. It is wired into PR CI and tag release workflows without secrets, external data, broker tokens, provider fetches, notifications, DB migrations, or trading-field semantic changes.
Rollback Notes: Remove the CI/release workflow step or set `SERENITY_RESEARCH_ENABLED=false`; revert DSA commit `d3127c5` to remove the release-check integration.
...
- [x] Release checklist 添加 flag off/on 验证。
- [x] CI 添加 Serenity unit tests。
- [x] CI 添加 report safety scanner。
- [x] CI 不要求外部数据源或真实 broker token。
- [x] release script 输出 machine-readable summary。
...
- [x] CI 可在无真实 secrets 环境运行。
- [x] Release checklist 明确 Serenity failure 的处理方式。
- [x] 发布流程包含 rollback command 或 revert PR 指引。
