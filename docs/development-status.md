...
> 最近可评审交付 checkpoint：本次实现 checkpoint 将由 `feat(P4): 实现调仓与目标权重` 生成；上一 checkpoint 为 `de50e5ff feat(P4): 实现公司行动入账`<br>
...
> 本次实现 checkpoint：待本次提交生成，标题为 `feat(P4): 实现调仓与目标权重`；已完成任务范围推进至 `SAL-P4-013`<br>
...
- 最近可评审交付 checkpoint：本次实现 checkpoint 将由 `feat(P4): 实现调仓与目标权重` 生成；上一 checkpoint：de50e5ff feat(P4): 实现公司行动入账
