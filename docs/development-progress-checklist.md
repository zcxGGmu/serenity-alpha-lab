...
| P5 Agent 与报告 | 13~16 | DOING | 7/18 | G5 | Evidence、引用、预算、可信报告 |
| P6 发布加固 | 16~18 | TODO | 0/23 | G6 | RC、稳定性、安全、发布与 Runbook |
| **合计** | **16~18 周** | **DOING** | **95/129** |  |  |
...
- [ ] [READY] 只消费技术/因子 Evidence，不让模型重算指标
...
当前已完成 `SAL-P0-001` 至 `SAL-P0-013`、`SAL-P1-001` 至 `SAL-P1-016`、`SAL-P2-001` 至 `SAL-P2-020`、`SAL-P3-001` 至 `SAL-P3-017`、`SAL-P4-001` 至 `SAL-P4-022`、`SAL-P5-001` 至 `SAL-P5-007`，完成度为 95/129；最近阶段性任务为 `SAL-P5-007` Agent Stage 持久化；最近可评审交付 checkpoint 为 `c12b81fe feat(P5): 实现 Agent Stage 持久化`，上一可评审交付 checkpoint 为 `cccc1416 feat(P5): 实现 Prompt 与输出 Schema Registry`，最近状态同步 checkpoint 为 `aac19354 docs: 同步 SAL-P5-007 checkpoint hash`，最新 hash-anchor checkpoint 为 `8d66ecf0 docs: 记录 SAL-P5-007 状态同步 hash`，最近最终 docs 固化 checkpoint 为 `f10b48f5 docs: 固化 SAL-P5-007 hash-anchor checkpoint`，最新状态复核 checkpoint 为 `bc4a491a docs: 复核 SAL-P5-007 最新开发状态与恢复提示`，上一状态复核 checkpoint 为 `f7fc3c80 docs: 复核 SAL-P4-022 最新开发状态与恢复提示`，上一 Gate checkpoint 为 `1466c11c docs(P4): 通过 Gate G4 回测与风控评审`。Gate G0、G1、G2、G3、G4 已通过（均为 `GO with accepted risks`），G5 未通过；下一步优先执行 `SAL-P5-008` Technical Agent 改造，暂不跳到 Intel/Risk/Decision Agent、Model routing、Citation Validator 或报告渲染任务；后续实现必须遵守 ADR-001/002、ADR-009、DEC-087、DEC-088、DEC-089、DEC-090、DEC-091、DEC-092、DEC-093 与 Gate G2/G3/G4，不得从 Agent Stage Store、Quant Evidence Adapter、source trust、Prompt Registry 或 EvidenceBundle 直接启动真实 Provider/LLM、Worker loop、Qlib runtime、生产调度、报告生成、正式组合回测推广或未经批准的大规模 DSA 源码迁移，且不得把 Qlib internal evidence、Dataset 转换、Screen result、AlphaSift T+N evaluation 或 legacy Signal Evaluation 命名为正式组合回测。
