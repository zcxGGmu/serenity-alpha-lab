...
> 上游候选基线：`ZhuLinsen/daily_stock_analysis v3.26.1`，正式实施以 Phase 0 锁定结果为准<br>
...
|---|---:|---|---:|---|---|
| P0 上游接管 | 1 | READY | 0/13 | G0 | DSA 可重复基线、金标、SBOM |
...
- [ ] [READY] 选择 DSA release/commit 并记录选择依据
- 元数据：优先级 P0 | 负责人 TL | 估算 0.5d | 实际 - | 依赖 -
...
- [ ] [TODO] 在当前仓库保留 DSA 历史并配置 `origin/upstream`
...
- [ ] [TODO] 构建并运行 DSA analyzer/server Docker Profile
- 元数据：优先级 P0 | 负责人 BE | 估算 1d | 实际 - | 依赖 SAL-P0-003
...
- [ ] [TODO] 生成 SBOM、许可证、漏洞和动态依赖报告
- 元数据：优先级 P0 | 负责人 SEC | 估算 1d | 实际 - | 依赖 SAL-P0-003
...
- [ ] [TODO] 决定是否正式采用 DSA 主干
- 元数据：优先级 P0 | 负责人 TL/RE/SEC | 估算 0.5d | 实际 - | 依赖 SAL-P0-001..012
...
- [ ] [TODO] 完成 ADR-001/002 及 Compatibility Facade 决策
...
- [ ] [TODO] 定义内容寻址 URI、Manifest、哈希和原子发布
...
- [ ] [TODO] 用 Protocol 隔离 DSA `AnalysisTaskQueue`
- 元数据：优先级 P0 | 负责人 BE | 估算 2d | 实际 - | 依赖 SAL-P1-006
...
- [ ] [TODO] 传播 `trace_id/run_id/stage_id`
- 元数据：优先级 P0 | 负责人 BE | 估算 1.5d | 实际 - | 依赖 SAL-P1-006
...
- [ ] [TODO] 让 Alembic 成为唯一 Schema 迁移入口
- 元数据：优先级 P0 | 负责人 BE | 估算 2d | 实际 - | 依赖 SAL-P0-009
...
- [ ] [TODO] 从脱敏 DSA fixture 执行 expand/backfill/verify
- 元数据：优先级 P0 | 负责人 BE | 估算 2d | 实际 - | 依赖 SAL-P1-012
...
- [ ] [TODO] 在新 lock/协议/迁移下重跑 DSA 主路径
- 元数据：优先级 P0 | 负责人 FE/BE | 估算 1d | 实际 - | 依赖 SAL-P1-003,SAL-P1-008,SAL-P1-013,SAL-P1-014
...
- [ ] [TODO] 用 InstrumentId 和 Symbol Mapping 包裹 `normalize_stock_code`
...
- [ ] [TODO] 保存可审计原始响应和请求元数据
- 元数据：优先级 P0 | 负责人 QE/BE | 估算 2d | 实际 - | 依赖 SAL-P1-007,SAL-P2-001
...
- [ ] [TODO] 构建历史有效期的 instrument master
- 元数据：优先级 P0 | 负责人 QE | 估算 2d | 实际 - | 依赖 SAL-P2-003,SAL-P2-004
...
|---|---|---|---|---|---|---|
| DEC-001 | - | DSA 正式基线 | 待 P0 决定 | ADR-001 | SAL-P0-001 | G0 |

## 14. 验收证据登记

| Evidence ID | 任务/Gate | 类型 | 路径/URL | commit/版本 | 评审人 | 日期 |
|---|---|---|---|---|---|---|
| AEV-001 | - | CI/报告/截图/Artifact/签字 | - | - | - | - |
...
首个可执行任务是 `SAL-P0-001`。在 Gate G0 前不应开始 Quant Core 或大规模重构；先确认 DSA 基线确实能够在目标环境中重复构建、测试、发布和长期同步。
