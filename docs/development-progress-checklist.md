...
| P6 发布加固 | 16~18 | DOING | 0/23 | G6 | RC、稳定性、安全、发布与 Runbook |
| **合计** | **16~18 周** | **DOING** | **106/129** |  |  |
...
当前已完成 `SAL-P0-001` 至 `SAL-P0-013`、`SAL-P1-001` 至 `SAL-P1-016`、`SAL-P2-001` 至 `SAL-P2-020`、`SAL-P3-001` 至 `SAL-P3-017`、`SAL-P4-001` 至 `SAL-P4-022`、`SAL-P5-001` 至 `SAL-P5-018`，完成度为 106/129；最近阶段性任务为 `SAL-P5-018` Gate G5 可信研究评审；最近可评审交付 checkpoint 为 `e65172b1 docs(P5): 通过 Gate G5 可信研究评审`；上一实现 checkpoint 为 `91d6d15b feat(P5): 建立 Agent 金标回归评测`；最新状态同步 checkpoint 为 `7d4d2e65 docs: 同步 SAL-P5-018 checkpoint hash`；上一状态同步 checkpoint 为 `4607532d docs: 同步 SAL-P5-017 checkpoint hash`；最新状态同步 hash-anchor checkpoint 为 `6e75580a docs: 记录 SAL-P5-017 状态同步 hash`；最新状态复核 checkpoint 为 `62cdcf23 docs: 复核 SAL-P5-017 最新开发状态与恢复提示`，最新状态复核 hash-anchor checkpoint 为 `839293a4 docs: 记录 SAL-P5-017 状态复核 hash`。Gate G0、G1、G2、G3、G4、G5 已通过（均为 `GO with accepted risks`）；下一步为 `SAL-P6-001` 完善认证与 RBAC，但本次不启动真实 Provider/LLM、Worker loop、Qlib runtime、生产调度、通知 sender、正式组合回测推广或其他 P6 后续任务；后续实现必须遵守 ADR-001/002、ADR-009、DEC-087 至 DEC-104 与 Gate G2/G3/G4/G5，不得从 Agent Stage Store、ModelInvocationPlanner、Prompt Registry、Citation Validator、AgentToolSecurityGuard、TrustedResearchReportRenderer、ResearchReportPagePresenter、NotificationOutboxStore、AgentEvaluationScorer、Quant Evidence Adapter、source trust、Technical/Intel/RiskPortfolio/Decision Agent adapter 或 EvidenceBundle 直接启动真实 Provider/LLM、Worker loop、Qlib runtime、生产调度、正式组合回测推广或未经批准的大规模 DSA 源码迁移，且不得把 Qlib internal evidence、Dataset 转换、Screen result、AlphaSift T+N evaluation 或 legacy Signal Evaluation 命名为正式组合回测。
