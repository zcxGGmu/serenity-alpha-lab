...
- [ ] [READY] 将 Screen/Model Signal 转为受约束订单
...
当前已完成 `SAL-P0-001` 至 `SAL-P0-013`、`SAL-P1-001` 至 `SAL-P1-016`、`SAL-P2-001` 至 `SAL-P2-020`、`SAL-P3-001` 至 `SAL-P3-017` 和 `SAL-P4-001` 至 `SAL-P4-012`，完成度为 78/129；最近阶段性任务为 `SAL-P4-012` 公司行动入账；最近实现 checkpoint 为 `de50e5ff feat(P4): 实现公司行动入账`，上一可评审交付 checkpoint 为 `30efc785 feat(P4): 实现 A 股执行规则`；最新状态同步 checkpoint 为 `c66555c3 docs: 同步 SAL-P4-012 checkpoint hash`，上一状态同步 checkpoint 为 `7c1e214c docs: 同步 SAL-P4-011 checkpoint hash`，最新状态同步 hash-anchor checkpoint 为 `07263d73 docs: 记录 SAL-P4-012 状态同步 hash`，上一 hash-anchor checkpoint 为 `9acf345d docs: 记录 SAL-P4-011 状态同步 hash`，最新最终锚点 checkpoint 为 `05acdb21 docs: 固化 SAL-P4-012 hash-anchor checkpoint`。Gate G0、G1、G2、G3 已通过（均为 `GO with accepted risks`），Gate G4 仍未通过。下一步优先执行 `SAL-P4-013` 调仓与目标权重；后续实现必须遵守 ADR-001/002、ADR-009、Gate G1/G2/G3 和 P4 入口约束，不得跳过调仓、Risk/Metric/Audit 等任务直接启动正式组合回测运行、Evidence Agent、真实 Provider/LLM、Risk/Quant Lab 或 Worker loop，且不得把 Qlib internal backtest evidence、Dataset 转换或 legacy Signal Evaluation 命名为正式组合回测。
