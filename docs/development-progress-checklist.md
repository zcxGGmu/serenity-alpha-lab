...
| P5 Agent 与报告 | 13~16 | DONE | 18/18 | G5 PASS | Evidence、引用、预算、可信报告 |
| P6 发布加固 | 16~18 | DOING | 3/23 | G6 | RC、稳定性、安全、发布与 Runbook |
| **合计** | **16~18 周** | **DOING** | **110/129** |  |  |
...
- [x] [DONE] 冻结证据、主张、引用和报告等级
- 元数据：优先级 P0 | 负责人 AI/BE | 估算 2d | 实际 0.5d | 依赖 SAL-P4-022 | 开始 2026-07-26 | 完成 2026-07-26
...
- 结果：新增 [Evidence / Claim / Report Schema](./evidence-claim-report-schema.md)、[schema.py](../src/serenity_alpha_lab/evidence/schema.py)、[test_evidence_schema_contract.py](../tests/evidence/test_evidence_schema_contract.py) 和 P5 architecture import guard；冻结 `research.evidence@1.0.0`、`research.claim@1.0.0`、`research.report@1.0.0` 与 `research.report_citation@1.0.0`，提供 Pydantic `model_json_schema()` / `evidence_json_schemas()` 和 `quant_evidence_source_matrix()`。
- 范围限制：本任务只定义 schema 和引用图验证，不实现 Evidence Store、EvidenceBundle Builder、Quant Evidence Adapter、Citation Validator、Agent stage、模型路由、报告渲染或通知；不启动 Evidence Agent、真实 Provider/LLM、Worker loop、Qlib runtime、生产调度或正式组合回测推广。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.evidence.schema` 而 `1 error`；Green focused target `5 passed`；Evidence + architecture related suite `20 passed`；完整验证记录见 AEV-089。

### SAL-P5-002 实现 Evidence Store

- [x] [DONE] 持久化不可变证据、修订和内容寻址正文
- 元数据：优先级 P0 | 负责人 BE/AI | 估算 2.5d | 实际 0.5d | 依赖 SAL-P5-001,SAL-P1-007 | 开始 2026-07-26 | 完成 2026-07-26
- 交付物：Repository、Artifact 正文、查询、去重和权限。
- 验收：
  - 相同内容哈希去重；修订产生新 Evidence。
  - 私有文档按用户/团队隔离。
- 结果：新增 [Evidence Store](./evidence-store.md)、Repository [evidence_store.py](../src/serenity_alpha_lab/repositories/evidence_store.py) 和 [Evidence Store contract test](../tests/repositories/test_evidence_store.py)，冻结 `research.evidence_body@1.0.0` 本地正文存储口径；`LocalEvidenceStore` 通过 P1 `ArtifactStore` 发布脱敏 canonical JSON body，持久化不可变 `EvidenceRecord` metadata、tenant/team/user scope、body artifact/hash 和 append-only revision link。
- 范围限制：本任务只实现本地 Evidence Store repository，不实现 EvidenceBundle Builder、Quant Evidence Adapter、Citation Validator、Agent stage、模型路由、报告渲染、通知、真实 Provider/LLM、Worker loop、Qlib runtime、生产调度或 DSA runtime source migration。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.repositories.evidence_store` 而 `1 error`；Green focused target `4 passed`；Evidence Store + Evidence schema + LocalArtifactStore + architecture related suite `28 passed`；完整验证记录见 AEV-090。
...
- [x] [DONE] 对公告、新闻、搜索和社交内容分级与去噪
- 元数据：优先级 P0 | 负责人 AI/SEC | 估算 2.5d | 实际 0.5d | 依赖 SAL-P5-002 | 开始 2026-07-26 | 完成 2026-07-26
- 交付物：TrustPolicy、URL/正文哈希、时间冲突、恶意标记。
- 验收：
  - 低可信来源不能单独支撑强结论。
  - 外部指令不进入系统 Prompt 或工具参数。
- 结果：新增 [Source Trust and Unstructured Cleaning](./source-trust-unstructured-cleaning.md)、Evidence [source_trust.py](../src/serenity_alpha_lab/evidence/source_trust.py) 和 [Source Trust contract test](../tests/evidence/test_source_trust_cleaning.py)，冻结 `research.source_trust@1.0.0` 离线来源信任与 prompt-safe 清洗口径；`SourceTrustPolicy` 对公告、监管披露、公司公告、新闻、搜索结果、社交和未知来源分配 trust，生成 canonical URL、URL/body SHA-256、time conflict issue、malicious instruction issue、corroboration guard 和 strong-claim guard。
- 范围限制：本任务只消费调用方提供的 source metadata 和 raw text，不抓取公告/新闻/搜索/社交内容，不执行 Evidence Agent、真实 Provider/LLM、Worker loop、Qlib runtime、生产调度、Citation Validator、Quant Evidence Adapter、报告渲染或正式组合回测推广。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.evidence.source_trust` 而 `1 error`；Green focused target `5 passed`；Source Trust + architecture related suite `21 passed`；完整验证记录见 AEV-092。
...
- [x] [DONE] 版本化角色 Prompt、工具、模型能力和输出契约
- 元数据：优先级 P0 | 负责人 AI | 估算 2d | 实际 0.5d | 依赖 SAL-P1-009,SAL-P5-001 | 开始 2026-07-27 | 完成 2026-07-27
- 交付物：Registry、发布状态、hash、变更对比。
- 验收：
  - 已发布 Prompt 不可原地修改。
  - Run 保存解析后的 Prompt/Schema/Tool 版本。
- 结果：新增 [Prompt and Output Schema Registry](./prompt-output-schema-registry.md)、Evidence [prompt_registry.py](../src/serenity_alpha_lab/evidence/prompt_registry.py) 和 [Prompt registry contract test](../tests/evidence/test_prompt_schema_registry.py)，冻结 `research.prompt_registry@1.0.0` 离线 Prompt/Schema/Tool/Model registry 口径；`PromptSchemaRegistry` 版本化 role prompts、output JSON Schemas、read-only tool declarations、model capability declarations 和 run prompt bindings，并记录 prompt/schema/tool/model canonical hashes。
- 范围限制：本任务只登记离线 metadata 与 hash，不执行 Evidence Agent、真实 Provider/LLM、Worker loop、Qlib runtime、生产调度、Citation Validator、报告渲染、Evidence Store 写入、EvidenceBundle 构建或正式组合回测推广。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.evidence.prompt_registry` 而 `1 error`；Green focused target `4 passed`；PromptRegistry + Evidence schema/source trust/quant adapter/bundle + architecture related suite `38 passed`；全量 pytest `432 passed, 3 skipped`；完整验证记录见 AEV-094。

### SAL-P5-007 实现 Agent Stage 持久化

- [x] [DONE] 为 DSA Orchestrator 增加 stage_id、attempt 和 checkpoint
- 元数据：优先级 P0 | 负责人 AI/BE | 估算 3d | 实际 0.5d | 依赖 SAL-P1-006,SAL-P1-009,SAL-P2-018 | 开始 2026-07-27 | 完成 2026-07-27
- 交付物：Stage Repository、恢复、取消、degrade/fail policy。
- 验收：
  - Worker 重启从最后成功阶段恢复。
  - 已成功的模型调用不重复收费。
- 结果：新增 [Agent Stage Persistence](./agent-stage-persistence.md)、Repository [agent_stage_store.py](../src/serenity_alpha_lab/repositories/agent_stage_store.py) 和 [Agent Stage Store contract test](../tests/repositories/test_agent_stage_store.py)，冻结 `research.agent_stage_store@1.0.0` 离线 Stage checkpoint 口径；`AgentStageStore` 持久化 deterministic `stage_id`、PromptRunBinding snapshot、stage checkpoint、model-call receipt、resume plan、cancel propagation 和 `degrade` / `skip` / `fail_run` policy。
- 范围限制：本任务只保存 Agent Stage metadata/hash 和模型调用成功回执，不执行 Evidence Agent、真实 Provider/LLM、Worker loop、Qlib runtime、生产调度、Citation Validator、报告渲染或正式组合回测推广；model routing/cache/budget enforcement 仍留给 `SAL-P5-012`。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.repositories.agent_stage_store` 而 `1 error`；Green focused target `4 passed`；AgentStage architecture guard `1 passed`；相关 TaskBackend/RunLifecycle/PromptRegistry/Architecture suite `54 passed`；全量 pytest `437 passed, 3 skipped`；完整验证记录见 AEV-095；实现 checkpoint 为 `c12b81fe feat(P5): 实现 Agent Stage 持久化`。

### SAL-P5-008 改造 Technical Agent

- [x] [DONE] 只消费技术/因子 Evidence，不让模型重算指标
- 元数据：优先级 P0 | 负责人 AI/QE | 估算 2d | 实际 0.5d | 依赖 SAL-P5-003,SAL-P5-005,SAL-P5-006 | 开始 2026-07-27 | 完成 2026-07-27
- 交付物：Prompt、Schema、兼容输出、回归 fixture。
- 验收：
  - 所有数值 Claim 有 citation。
  - 原 DSA Dashboard 字段在兼容层可继续生成。
- 结果：新增 [Technical Agent Evidence Adapter](./technical-agent-evidence-adapter.md)、Application [technical_agent.py](../src/serenity_alpha_lab/application/technical_agent.py) 和 [Technical Agent adapter contract test](../tests/application/test_technical_agent_evidence_adapter.py)，冻结 `research.agent.technical@1.0.0` 离线 Technical Agent 证据适配口径；`EvidenceScopedTechnicalAgent` 只接收 technical `EvidenceBundle` 与 concrete `PromptRunBinding`，按 kind+scope+`llm_recompute_allowed=false` 拒绝 formal backtest/risk/audit/API/UI lineage evidence，校验 numeric Claim citation lineage/value 一致，并输出 DSA 兼容 opinion/dashboard 与 `data_perspective` 字段。
- 范围限制：本任务只准备 Technical Agent prompt payload 与验证结构化输出，不调用真实 Provider/LLM、不执行 DSA Technical Agent tools、不读取 Evidence body、不写 Evidence Store、不启动 Worker loop/Qlib runtime/生产调度/Citation Validator/报告渲染或正式组合回测推广；Intel/Risk/Portfolio/Decision Agent、model routing/cache/budget 和 Citation Validator 仍留给后续任务。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.application.technical_agent` 而 `1 error`；review 回归 Red target 捕获 evidence scope/recompute、numeric citation mismatch 和 `data_perspective` 缺口（`3 failed, 4 passed`）；Green focused target `7 passed`；TechnicalAgent architecture guard `1 passed`；相关 TechnicalAgent/EvidenceBundle/QuantEvidenceAdapter/PromptRegistry/AgentStageStore/Architecture suite `41 passed`；全量 pytest `445 passed, 3 skipped`；完整验证记录见 AEV-096；实现 checkpoint 为 `74701974 feat(P5): 改造 Technical Agent`。

### SAL-P5-009 改造 Intel Agent

- [x] [DONE] 增加来源、发布时间、可信度和冲突处理
- 元数据：优先级 P0 | 负责人 AI | 估算 2.5d | 实际 0.5d | 依赖 SAL-P5-003,SAL-P5-004,SAL-P5-006 | 开始 2026-07-27 | 完成 2026-07-27
- 交付物：搜索结果 Adapter、Prompt、结构化事件输出。
- 验收：
  - 事件时间与抓取时间分离。
  - 重复、陈旧和恶意内容被标记或排除。
- 结果：新增 [Intel Agent Evidence Adapter](./intel-agent-evidence-adapter.md)、Application [intel_agent.py](../src/serenity_alpha_lab/application/intel_agent.py) 和 [Intel Agent adapter contract test](../tests/application/test_intel_agent_evidence_adapter.py)，冻结 `research.agent.intel@1.0.0` 离线 Intel Agent 证据适配口径；新增 `unstructured_source` / `market_intelligence` Evidence 表示，`EvidenceScopedIntelAgent` 只接收 intel `EvidenceBundle` 与 concrete `PromptRunBinding`，消费 `metadata.source_trust` prompt-safe 记录，分离 event/published/observed/available timestamps，排除 duplicate/malicious source，标记 stale/low-trust source，并输出 DSA 兼容 news/intel fields。
- 范围限制：本任务只准备 Intel Agent prompt payload 与验证结构化事件输出，不抓取新闻/搜索/社交内容，不调用真实 Provider/LLM，不执行 DSA Intel tools，不读取 Evidence body，不写 Evidence Store，不启动 Worker loop/Qlib runtime/生产调度/Citation Validator/报告渲染或正式组合回测推广；Risk/Portfolio/Decision Agent、model routing/cache/budget 和 Citation Validator 仍留给后续任务。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.application.intel_agent` 而 `1 error`；Green focused target `4 passed`；IntelAgent architecture guard `1 passed`；相关 IntelAgent/EvidenceBundle/SourceTrust/PromptRegistry/EvidenceSchema/AgentStageStore/Architecture suite `47 passed`；全量 pytest `451 passed, 3 skipped`；完整验证记录见 AEV-097；实现 checkpoint 为 `a6974362 feat(P5): 改造 Intel Agent`。

### SAL-P5-010 改造 Risk/Portfolio Agent

- [x] [DONE] 消费确定性 Risk/Portfolio 结果且无权覆盖硬门禁
- 元数据：优先级 P0 | 负责人 AI/QE | 估算 2d | 实际 0.5d | 依赖 SAL-P4-014,SAL-P5-005,SAL-P5-006 | 开始 2026-07-28 | 完成 2026-07-28
- 交付物：Prompt、Schema、risk override 防护、回归。
- 验收：
  - block/not-evaluable 在最终结果保持。
  - Agent 只补充非结构化风险和解释。
- 结果：新增 [Risk/Portfolio Agent Evidence Adapter](./risk-portfolio-agent-evidence-adapter.md)、Application [risk_portfolio_agent.py](../src/serenity_alpha_lab/application/risk_portfolio_agent.py) 和 [Risk/Portfolio Agent adapter contract test](../tests/application/test_risk_portfolio_agent_evidence_adapter.py)，冻结 `research.agent.risk_portfolio@1.0.0` 离线 Risk/Portfolio Agent 证据适配口径；`EvidenceScopedRiskPortfolioAgent` 只接收 `risk_portfolio` EvidenceBundle 与 concrete PromptRunBinding，allowlist 限定 formal portfolio backtest risk/metrics/audit/run/artifact/API evidence，并要求 `llm_recompute_allowed=false`。Prompt payload 保留 hard gate summary，`block`、`not_evaluable`、`eligible_for_ranking=false` 和 `agent_strong_conclusion_allowed=false` 不允许被 Agent 输出升级；结构化输出必须使用 current bundle citation graph，numeric Claim 必须为 deterministic evidence 且与 citation value/unit/formula/dataset/run/stage/artifact 一致；兼容层输出 legacy DSA `risk_portfolio` opinion/dashboard fields。
- 范围限制：本任务只准备 Risk/Portfolio Agent prompt payload 与验证结构化输出，不调用真实 Provider/LLM、不执行 DSA Risk/Portfolio tools、不读取 Evidence body、不写 Evidence Store、不启动 Worker loop/Qlib runtime/生产调度/Citation Validator/报告渲染或正式组合回测推广；Decision Agent、model routing/cache/budget 和 Citation Validator 仍留给后续任务。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.application.risk_portfolio_agent` 而 `1 error`；Green focused target `4 passed`；RiskPortfolio architecture guard `1 passed`；相关 RiskPortfolioAgent/EvidenceBundle/QuantEvidenceAdapter/PromptRegistry/AgentStageStore/Architecture suite `41 passed`；全量 pytest `456 passed, 3 skipped`；完整验证记录见 AEV-098；实现 checkpoint 为 `22ecff19 feat(P5): 改造 Risk Portfolio Agent`。

### SAL-P5-011 实现多空反证与最终综合

- [x] [DONE] 增加 Bull/Bear/Disagreement/Decision 受控流程
- 元数据：优先级 P1 | 负责人 AI/RE | 估算 3d | 实际 0.5d | 依赖 SAL-P5-008..010 | 开始 2026-07-28 | 完成 2026-07-28
- 交付物：角色 Stage、冲突摘要、失效条件和置信等级。
- 验收：
  - 多空不能只重复分析员摘要。
  - Final Decision 不新增前序 Evidence 中不存在的事实。
- 结果：新增 [Decision Agent Counterargument and Final Synthesis](./decision-agent-counterargument-synthesis.md)、Application [decision_agent.py](../src/serenity_alpha_lab/application/decision_agent.py) 和 [Decision Agent counterargument contract test](../tests/application/test_decision_agent_counterargument_synthesis.py)，冻结 `research.agent.decision@1.0.0` 离线 Decision Agent 综合口径；`EvidenceScopedDecisionAgent` 只消费已验证的 Technical/Intel/RiskPortfolio result、decision EvidenceBundle 与 concrete PromptRunBinding，生成 deterministic prompt payload、role result hashes、prior-role citation evidence map 和 preserved risk gate summary。
- 范围限制：本任务只准备 Decision prompt payload 与验证结构化 Decision 输出，不调用真实 Provider/LLM、不执行 DSA tools、不读取 Evidence body、不写 Evidence Store、不启动 Worker loop/Qlib runtime/生产调度/Citation Validator/报告渲染或正式组合回测推广；model routing/cache/budget 和 Citation Validator 仍留给后续任务。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.application.decision_agent` 而 `1 error`；Green focused target `4 passed`；Decision architecture guard `1 passed`；相关 Decision/Technical/Intel/RiskPortfolio/EvidenceBundle/PromptRegistry/AgentStageStore/Architecture suite `54 passed`；全量 pytest `461 passed, 3 skipped`；完整验证记录见 AEV-099；实现 checkpoint 为 `50e6aa39 feat(P5): 实现多空反证与最终综合`。

### SAL-P5-012 完善模型路由、缓存与预算

- [x] [DONE] 在 DSA LiteLLM 上实现调用/Run/日三级预算
- 元数据：优先级 P0 | 负责人 AI/BE | 估算 2.5d | 实际 0.5d | 依赖 SAL-P5-006,SAL-P5-007 | 开始 2026-07-28 | 完成 2026-07-28
- 交付物：ModelInvocation、精确缓存、价格表、限流和降级。
- 验收：
  - 超预算返回 partial/budget-exhausted，不静默继续。
  - cache key 包含 Evidence/Prompt/Model/参数版本。
- 结果：新增 [Model Routing, Cache and Budget](./model-routing-cache-budget.md)、Application [model_routing.py](../src/serenity_alpha_lab/application/model_routing.py) 和 [Model routing/cache/budget contract test](../tests/application/test_model_routing_cache_budget.py)，冻结 `research.model_routing@1.0.0` 离线模型调用规划边界；`ModelInvocationPlanner` 只消费 EvidenceBundle、PromptRunBinding、版本化参数、调用方提供的候选 route、离线价格表、预算 usage snapshot 和成功 receipt metadata，输出 `cache_hit` / `ready` / `degraded` / `budget_exhausted` / `rate_limited` deterministic plan。
- 范围限制：本任务只计算模型路由、精确缓存键、价格表成本估算、调用/Run/日预算和限流降级计划；不导入 LiteLLM、不调用真实 Provider/LLM、不执行 Agent stage、不写 AgentStageStore receipt、不读 Evidence body、不写 Evidence Store、不启动 Worker loop/Qlib runtime/生产调度/Citation Validator/报告渲染或正式组合回测推广。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.application.model_routing` 而 `1 error`；Green focused target `4 passed`；ModelRouting architecture guard `1 passed`；相关 ModelRouting/PromptRegistry/EvidenceBundle/AgentStageStore/Architecture suite `40 passed`；全量 pytest `466 passed, 3 skipped`；完整验证记录见 AEV-100；实现 checkpoint 为 `83ae4310 feat(P5): 实现模型路由缓存与预算`。

### SAL-P5-013 实现 Citation Validator

- [x] [DONE] 校验证据存在、数值一致、时间和方向
- 元数据：优先级 P0 | 负责人 AI/QE | 估算 3d | 实际 0.5d | 依赖 SAL-P5-001,SAL-P5-005,SAL-P5-008..011 | 开始 2026-07-28 | 完成 2026-07-28
- 交付物：Claim Validator、修复流程、报告等级。
- 验收：
  - 数值/日期/比率/价格目标强制引用。
  - 一次修复后仍失败的 Claim 被删除并降级，不能伪装 verified。
- 结果：新增 [Citation Validator](./citation-validator.md)、Evidence [citation_validator.py](../src/serenity_alpha_lab/evidence/citation_validator.py) 和 [Citation Validator contract test](../tests/evidence/test_citation_validator.py)，冻结 `research.citation_validator@1.0.0` 离线引用校验边界；`CitationValidator` 只消费已构造的 `ResearchReport`、`ResearchClaim`、`ReportCitation` 和 `EvidenceRecord`，校验 mandatory citation、claim/citation/evidence value/unit/formula/dataset/run/stage/artifact lineage、decision-time availability 和一次 caller-supplied repair 后仍失败 Claim 删除降级。
- 范围限制：本任务只执行离线结构化引用校验，不读取 Evidence body、不调用真实 Provider/LLM、不执行 Agent stage、不写 Evidence Store、不启动 Worker loop/Qlib runtime/生产调度、报告渲染或正式组合回测推广。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.evidence.citation_validator` 而 `1 error`；local review regression Red 捕获 citation/evidence lineage issue 未挂接到 consuming claim（`1 failed, 4 passed`）；Green focused target `7 passed`，包含 missing cited evidence 与 future evidence invalid graph 回归；CitationValidator architecture guard `1 passed`；相关 CitationValidator/EvidenceSchema/QuantEvidenceAdapter/Technical/Intel/RiskPortfolio/Decision/Architecture suite `59 passed`；全量 pytest `474 passed, 3 skipped`；完整验证记录见 AEV-101；实现 checkpoint 为 `dfd82553 feat(P5): 实现 Citation Validator`。

### SAL-P5-014 强化 Agent 工具安全

- [x] [DONE] 默认 deny、参数 Schema、SSRF 和 Prompt Injection 防护
- 元数据：优先级 P0 | 负责人 SEC/AI | 估算 2.5d | 实际 0.5d | 依赖 SAL-P5-004,SAL-P5-006 | 开始 2026-07-28 | 完成 2026-07-28
- 交付物：Tool allowlist、权限/副作用声明、攻击测试集。
- 验收：
  - Agent 无 Shell、交易和任意数据库写权限。
  - 恶意网页不能改变系统规则或发起未授权 URL。
- 结果：新增 [Agent Tool Security](./agent-tool-security.md)、Application [agent_tool_security.py](../src/serenity_alpha_lab/application/agent_tool_security.py) 和 [Agent tool security contract test](../tests/application/test_agent_tool_security.py)，冻结 `research.agent_tool_security@1.0.0` 离线工具授权口径；`AgentToolSecurityGuard` 只消费 concrete `PromptRunBinding`、调用方 stage tool allowlist 和调用方参数，默认 deny 未绑定/未授权工具，校验 JSON-Schema subset，阻止 shell/trading/database/filesystem write 类 scope，拦截 SSRF/未授权 URL 和 prompt-injection 参数。
- 范围限制：本任务只返回 deterministic allow/deny 决策和 safe argument copy，不执行工具、不调用真实 Provider/LLM、不读 Evidence body、不写 Evidence Store、不启动 Worker loop/Qlib runtime/生产调度、Citation Validator repair、报告渲染或正式组合回测推广。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.application.agent_tool_security` 而 `1 error`；Green focused target `4 passed`；AgentToolSecurity architecture guard `1 passed`；相关 AgentToolSecurity/PromptRegistry/SourceTrust/ModelRouting/AgentStageStore/Architecture suite `47 passed`；全量 pytest `479 passed, 3 skipped`；compileall PASS；dependency lock guard PASS with `Resolved 298 packages`；immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`；`git diff --check` PASS；完整验证记录见 AEV-102。

### SAL-P5-015 实现可信 ResearchReport 与 Renderer

- [x] [DONE] 渲染 verified/partial/insufficient-evidence 报告
- 元数据：优先级 P0 | 负责人 AI/FE | 估算 2.5d | 实际 0.5d | 依赖 SAL-P5-013 | 开始 2026-07-29 | 完成 2026-07-29
- 交付物：权威 JSON、Markdown/HTML Renderer、模板版本。
- 验收：
  - Markdown 不是权威数据源。
  - 报告显示 as-of、Dataset、模型、成本、风险和免责声明。
- 结果：新增 [Trusted ResearchReport Renderer](./trusted-research-report-renderer.md)、Evidence [report_renderer.py](../src/serenity_alpha_lab/evidence/report_renderer.py) 和 [Report Renderer contract test](../tests/evidence/test_report_renderer.py)，冻结 `research.report_renderer@1.0.0` 离线可信报告渲染口径；`TrustedResearchReportRenderer` 只消费结构化 `ResearchReport` 和可选 `CitationValidationResult`，输出 `authority=canonical_json` 的权威 JSON envelope、deterministic `authoritative_json_hash` 以及从该 envelope 派生的 Markdown/HTML，显示 report level、as-of、Dataset、模型、成本、风险、免责声明、claims、citations、evidence lineage 和 validation issues。
- 范围限制：本任务不实现 UI/通知/Outbox、不解析 Markdown 为权威数据、不调用真实 Provider/LLM、不执行 Agent stage/tool、不读 Evidence body、不写 Evidence Store、不启动 Worker loop/Qlib runtime/生产调度、Citation Validator repair 或正式组合回测推广。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.evidence.report_renderer` 而 `1 error`；Green focused target `4 passed`；ReportRenderer architecture guard `1 passed`；相关 P5 renderer suite `73 passed`；全量 pytest `484 passed, 3 skipped`；compileall PASS；dependency lock guard PASS with `Resolved 298 packages`；immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`；完整验证记录见 AEV-103。

### SAL-P5-016 实现引用 UI 与通知 Outbox

- [x] [DONE] 在 DSA Web/桌面/通知展示证据和发送状态
- 元数据：优先级 P0 | 负责人 FE/BE | 估算 3d | 实际 0.5d | 依赖 SAL-P5-015 | 开始 2026-07-29 | 完成 2026-07-29
- 交付物：引用展开、来源链接、Report 页面、Transactional Outbox。
- 验收：
  - 页面可从 Claim 展开到 Evidence/Artifact。
  - 通知至少一次发送但按 dedupe_key 不重复。
- 结果：新增 [Research Report Delivery UI And Notification Outbox](./research-report-delivery-ui-outbox.md)、Application [report_delivery.py](../src/serenity_alpha_lab/application/report_delivery.py)、Repository [notification_outbox.py](../src/serenity_alpha_lab/repositories/notification_outbox.py) 和 DSA Web extension patch [0007-add-research-report-delivery-ui.patch](../patches/dsa/v3.26.1/0007-add-research-report-delivery-ui.patch)，冻结 `research.report_delivery_ui@1.0.0` 与 `research.notification_outbox@1.0.0`；Presenter 只消费 `RenderedResearchReport.trusted_report.authoritative_json`，输出 claim -> citation -> evidence -> source/artifact 展开 payload，Outbox 以 `(tenant_id, channel, dedupe_key)` 幂等入队并支持 pending/sending/sent/dead_letter lease/retry 状态。
- 范围限制：本任务只实现离线 UI payload、SQLAlchemy Outbox metadata 和 DSA Web GET-only 页面；不实现通知 sender、不注册后端 runtime route、不启动 Agent 评测、Gate G5、真实 Provider/LLM、Worker loop、Qlib runtime、生产调度或正式组合回测推广。
- 验收证据：Red target 初始因缺少 `serenity_alpha_lab.application.report_delivery` 与 `serenity_alpha_lab.repositories.notification_outbox` 而失败；focused Python/architecture target `6 passed`；DSA Web focused target `4 passed files / 26 passed tests`；`npm run lint` PASS；`npm run build` PASS；相关 P5/report/outbox suite `79 passed`；全量 pytest `490 passed, 3 skipped`；compileall PASS；dependency lock guard PASS with `Resolved 298 packages`；clean temp DSA worktree 顺序应用 `0001..0007`；immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`；完整验证记录见 AEV-104。

### SAL-P5-017 建立 Agent 金标与回归评测

- [x] [DONE] 完成 50+ 案例、指标、切分和模型/Prompt 对比
- 元数据：优先级 P0 | 负责人 AI/RE/SEC | 估算 3d | 实际 0.5d | 依赖 SAL-P5-008..016 | 开始 2026-07-30 | 完成 2026-07-30
- 交付物：评测集、离线 Stub、评分器、回归报告。
- 结果：新增 [Agent Golden Regression Evaluation](./agent-golden-regression-evaluation.md)、Application [agent_evaluation.py](../src/serenity_alpha_lab/application/agent_evaluation.py) 和 [Agent golden regression tests](../tests/application/test_agent_golden_regression_evaluation.py)，冻结 `research.agent_evaluation@1.0.0` 离线评测边界；默认 catalog 含 `56` 个金标案例，覆盖 normal/missing data/financial anomaly/major event/viewpoint conflict/malicious content/multi-market，每类 `8` 个案例，跨 CN/HK/US/JP/KR/TW 市场；离线 stub baseline 得分为引用准确率 `1.0`、无依据数值率 `0.0`、schema 成功率 `1.0`、安全核心集全通过。
- 范围限制：本任务只建立 offline evaluation catalog/stub/scorer/report，不执行真实 Agent stage、不调用真实 Provider/LLM、不启动 Worker loop、Qlib runtime、生产调度、Gate G5 或正式组合回测推广。
- 验收：
  - 覆盖缺失、异常、事件、冲突、恶意内容和多市场样本。
  - 引用准确率 >=95%，无依据数值率 <1%，安全核心集全通过。
  - 验收证据：Red target 初始因缺少 `serenity_alpha_lab.application.agent_evaluation` 而 `1 error`；focused target `5 passed`；相关 P5 suite `76 passed`；全量 pytest `495 passed, 3 skipped`；完整验证记录见 AEV-105。

### SAL-P5-018 Gate G5：可信研究评审

- [x] [DONE] 批准证据化 Agent 和报告进入 RC
- 元数据：优先级 P0 | 负责人 TL/AI/RE/SEC | 估算 0.5d | 实际 0.5d | 依赖 SAL-P5-001..017 | 开始 2026-07-30 | 完成 2026-07-30
- 交付物：Gate 记录、评测、成本和攻击测试报告。
- 结果：新增 [Gate G5 Trusted Research Review](./gate-g5-trusted-research-review.md)、[Gate G5 integration test](../tests/gates/test_gate_g5_trusted_research_review.py) 和实施计划 [2026-07-30-gate-g5-trusted-research-review.md](./superpowers/plans/2026-07-30-gate-g5-trusted-research-review.md)；Gate 结论为 `GO with accepted risks`，P5 完成 `18/18`，总进度推进至 `106/129`，允许进入 `SAL-P6-001` 完善认证与 RBAC。
- 范围限制：Gate G5 只批准离线可信研究链进入 P6 RC hardening；不调用真实 Provider/LLM、不启动 Worker loop、不启动 Qlib runtime、不启动生产调度、不推广正式组合回测、不实现通知 sender、不注册生产后端 route，也不提前执行 P6 任务。
- 验收：
  - Evidence、checkpoint、预算、引用、报告等级和 UI 全链路通过。
  - Agent 无法覆盖 Quant/Risk 硬事实与门禁。
  - 验收证据：Red target `1 failed, 1 passed`，缺少 `docs/gate-g5-trusted-research-review.md`；Green focused target `2 passed`；相关 P5 suite `78 passed`；全量 pytest `497 passed, 3 skipped`；compileall PASS；dependency lock guard PASS；live patched DSA worktree `--check-only` 命中 expected already-applied context conflict，clean temp DSA worktree 顺序应用 `0001..0007` PASS；immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`；`git diff --check` PASS；完整验证记录见 AEV-106。

## 9. Phase 6：安全、稳定性与发布加固

### SAL-P6-001 完善认证与 RBAC

- [x] [DONE] 定义 desktop/standalone/team 的身份和权限模型
- 元数据：优先级 P0 | 负责人 BE/SEC | 估算 2.5d | 实际 0.5d | 依赖 SAL-P5-018 | 开始 2026-07-30 | 完成 2026-07-30
- 交付物：角色、权限矩阵、OIDC 可选接入、API 测试。
- 验收：
  - desktop 本地模式不降低易用性。
  - team 模式的数据、运行、配置和管理权限分离。
- 结果：新增 [Auth And RBAC](./auth-rbac.md)、Application [auth_rbac.py](../src/serenity_alpha_lab/application/auth_rbac.py) 和 [Auth/RBAC contract test](../tests/application/test_auth_rbac.py)，冻结 `security.auth_rbac@1.0.0` framework-neutral 身份与权限契约；定义 desktop/standalone/team 三类模式、`AuthSubject`、`ResourceScope`、角色/权限矩阵、`RbacPolicy`、可选 OIDC provider declaration、claim mapping 和 API authorization catalog。Desktop local owner 不要求 OIDC 或 team setup；team mode 将 data、run、config、admin 权限分离，tenant/team mismatch 默认 deny。
- 边界：本任务不实现 `SAL-P6-002` 对象级签名 URL/owner policy，不接入 Secret Manager、SSRF/file upload hardening、SCA gate、OpenTelemetry、Worker loop、真实 Provider/LLM、Qlib runtime、notification sender、production scheduler、release packaging 或正式组合回测推广。
- 验收证据：Red target missing module `1 error`；focused Green `5 passed`；architecture guard `1 passed`；related suite `41 passed`；full pytest `503 passed, 3 skipped`；compileall/lock/tag/diff checks PASS；见 `AEV-107`。

### SAL-P6-002 实现资源与 Artifact 授权

- [x] [DONE] 对 Run、Definition、Evidence、Report 和下载实施对象级权限
- 元数据：优先级 P0 | 负责人 BE/SEC | 估算 2.5d | 实际 0.5d | 依赖 SAL-P6-001 | 开始 2026-07-30 | 完成 2026-07-30
- 交付物：owner/tenant、授权检查、短时签名 URL、审计。
- 结果：新增 [Resource And Artifact Authorization](./resource-artifact-authorization.md)、Application [resource_authorization.py](../src/serenity_alpha_lab/application/resource_authorization.py) 和 [Resource authorization contract test](../tests/application/test_resource_authorization.py)，冻结 `security.resource_artifact_authorization@1.0.0` framework-neutral 对象级授权口径；`ResourceAuthorizationPolicy` 复用 `SAL-P6-001` RBAC 后继续校验 tenant/team/private owner scope，Artifact download 必须绑定 parent Run/Definition/Evidence/Report read access，`SignedArtifactUrlIssuer` 签发短时 HMAC URL 并拒绝篡改、重复签名参数与过期，`WorkerResourceGrant` 只授予 run-bound task-scoped `run:read` 与显式 Artifact ids。
- 边界：本任务不接入 Secret Manager/OS Keychain、不注册 FastAPI middleware、不连接对象存储、不启动真实 Provider/LLM、Worker loop、Qlib runtime、notification sender、production scheduler、release packaging 或正式组合回测推广。
- 验收：
  - 不能通过猜测 ID 或 URI 跨用户读取。
  - 后台 Worker 只获得任务所需最小权限。
  - 验收证据：Red target missing module `1 error`；architecture Red missing file `1 failed`；focused Green `6 passed`；architecture guard `1 passed`；related suite `62 passed`；full pytest `510 passed, 3 skipped`；compileall PASS；dependency lock guard PASS；`git diff --check` PASS；immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`；见 `AEV-108`。

### SAL-P6-003 加固密钥与配置

- [x] [DONE] 接入 Secret Manager/OS Keychain 和配置审计
- 元数据：优先级 P0 | 负责人 SEC/BE | 估算 2d | 实际 0.5d | 依赖 SAL-P1-014,SAL-P6-001 | 开始 2026-07-31 | 完成 2026-07-31
- 交付物：密钥引用、轮换、脱敏诊断、访问审计。
- 结果：新增 [Secret And Config Hardening](./secret-config-hardening.md)，扩展 Application [config_profiles.py](../src/serenity_alpha_lab/application/config_profiles.py) 和 [Config profile contract tests](../tests/application/test_config_profiles.py)，冻结 metadata-only Secret Manager/OS Keychain 引用、`config_api_diagnostics()`、`SecretRotationPlan` 和 `ConfigAuditRecord` 口径；`SecretReference` 只保存 `env://`、`keychain://` 或 `secretmanager://` 引用、存在状态、last-four、版本和 reference hash，拒绝 raw-looking plaintext secret、query/fragment 和不匹配 backend scheme。
- 边界：本任务不导入真实 Keychain/Secret Manager/cloud SDK，不注册 FastAPI config route，不写 `.env`/数据库/备份，不启动真实 Provider/LLM、Worker loop、Qlib runtime、notification sender、production scheduler、release packaging 或正式组合回测推广。
- 验收：
  - 数据库、日志、Trace、前端和备份无明文密钥。
  - 配置 API 只显示存在状态和最后四位。
  - 验收证据：Red target missing `ConfigAuditAction` import `1 error`；focused Green `13 passed`；architecture guard `1 passed`；related Config/Auth/Resource/Architecture suite `57 passed`；full pytest `515 passed, 3 skipped`；compileall PASS；dependency lock guard PASS；immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`；见 `AEV-109`。

### SAL-P6-004 加固输入、抓取与报告渲染

- [x] [DONE] 实施上传限制、SSRF、防恶意 Markdown/HTML
- 元数据：优先级 P0 | 负责人 SEC/BE/FE | 估算 3d | 实际 0.5d | 依赖 SAL-P5-014,SAL-P5-015 | 开始 2026-07-31 | 完成 2026-07-31
- 交付物：URL Policy、文件扫描、sanitizer、安全 Header。
- 结果：新增 [Input Fetch And Report Rendering Hardening](./input-fetch-report-hardening.md)、Application [input_fetch_security.py](../src/serenity_alpha_lab/application/input_fetch_security.py) 和 [input/fetch/report security contract tests](../tests/application/test_input_fetch_report_security.py)，冻结 `security.input_fetch_report_hardening@1.0.0` framework-neutral 输入、抓取与报告渲染安全口径；`UrlFetchPolicy` 只消费调用方提供的 URL、redirect、resolved IP、response content-type/size metadata，拒绝危险协议、credentials、本机/私网/metadata IP、未批准 host、重定向绕过和超限响应；`FileUploadPolicy` 只消费文件名、content-type、size 和 sample bytes，拒绝 traversal、超限、危险类型和 executable/script signatures；`ReportRenderSecurityPolicy` 阻止 active HTML/unsafe links，`ResearchReportPagePresenter` 在保留 canonical JSON authority 的同时过滤 unsafe source links 并附加 CSP/security headers。
- 边界：本任务不执行 live web/search/social/browser/DNS/HTTP fetch，不实现真实上传 endpoint、存储 adapter、杀毒引擎、FastAPI route/middleware 或前端页面，不调用真实 Provider/LLM，不执行 Agent/tool、Citation repair、Evidence Store write、Worker loop、Qlib runtime、notification sender、production scheduler、release packaging 或 formal backtest promotion。
- 验收：
  - 私网/本机地址、危险协议和重定向绕过被阻止。
  - 报告不能执行脚本或注入不安全链接。
  - 验收证据：Red target missing `serenity_alpha_lab.application.input_fetch_security` `1 error`；focused Green `6 passed`；architecture/focused security `8 passed`；related SourceTrust/AgentToolSecurity/Renderer/Delivery/Architecture suite `53 passed`；full pytest `520 passed, 3 skipped`；compileall PASS；dependency lock guard PASS；`git diff --check` PASS；immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`；见 `AEV-110`。
...
| RSK-003 | Agent 引用/幻觉 | 高 | 高 | 无依据数字/错引 | 已由 P5 离线可信研究链关闭 RC 输入边界：Evidence/Claim/Report Schema、EvidenceBundle、Quant Evidence Adapter、Citation Validator、Agent Tool Security、TrustedResearchReport Renderer、Research Report Delivery UI 和 56-case Agent Golden regression 共同覆盖错引、无依据数值、恶意内容和 hard-gate override；真实 Provider/LLM runtime 质量风险转入 P6 profile/observability/worker 门禁管理 | AI/RE/SEC | G5 | CLOSED |
...
| DEC-092 | 2026-07-27 | Prompt 与输出 Schema Registry 口径 | 采用 `evidence.prompt_registry.PromptSchemaRegistry` 冻结 `research.prompt_registry@1.0.0` 离线 Prompt/Schema/Tool/Model registry 边界；Registry 版本化 `technical`、`intel`、`risk_portfolio`、`decision` role prompts、output JSON Schemas、read-only tool declarations、model capability declarations 和 `PromptRunBinding`，并为 prompt/schema/tool/model/binding 生成 canonical SHA-256。已发布 Prompt 不可原地覆盖，minor output schema 仅允许新增 optional property，run binding 必须保存 concrete prompt/schema/tool/model versions 与 hashes，`latest` alias 被拒绝；默认 prompts 明确 `no_llm_recompute`，只允许引用 EvidenceBundle evidence ids/hashes。本任务不执行 Evidence Agent、真实 Provider/LLM、Worker loop、Qlib runtime、Citation Validator、报告渲染、生产调度或正式组合回测推广 | [prompt-output-schema-registry.md](./prompt-output-schema-registry.md); [prompt_registry.py](../src/serenity_alpha_lab/evidence/prompt_registry.py); [test_prompt_schema_registry.py](../tests/evidence/test_prompt_schema_registry.py) | SAL-P5-006,SAL-P5-007,SAL-P5-008,SAL-P5-009,SAL-P5-010,SAL-P5-012,SAL-P5-014 | G5 |
| DEC-093 | 2026-07-27 | Agent Stage 持久化口径 | 采用 `repositories.agent_stage_store.AgentStageStore` 冻结 `research.agent_stage_store@1.0.0` 离线 Stage checkpoint 边界；`stage_id` 由 `run_id`、`stage_name`、`input_hash` 和 concrete `prompt_version` deterministic hash 生成，Stage registration 必须绑定匹配的 `PromptRunBinding` run/stage/role/prompt version。Stage checkpoint 只保存 metadata/hash、Prompt/Schema/Tool/Model binding hash、failure policy、attempt、output hash 和 model-call receipt；resume plan 对 `succeeded/degraded/skipped` 返回 `skip_reused`，对 running 且已有成功 receipt 返回 `reuse_model_call`，对 failed/cancelled 返回停止动作。本任务不执行 Agent、真实 Provider/LLM、Worker loop、Qlib runtime、model budget/cache、Citation Validator、报告渲染或正式组合回测推广 | [agent-stage-persistence.md](./agent-stage-persistence.md); [agent_stage_store.py](../src/serenity_alpha_lab/repositories/agent_stage_store.py); [test_agent_stage_store.py](../tests/repositories/test_agent_stage_store.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py) | SAL-P5-007,SAL-P5-008,SAL-P5-009,SAL-P5-010,SAL-P5-012,SAL-P5-017 | G5 |
| DEC-094 | 2026-07-27 | Technical Agent 证据适配口径 | 采用 `application.technical_agent.EvidenceScopedTechnicalAgent` 冻结 `research.agent.technical@1.0.0` 离线 Technical Agent adapter 边界；adapter 只消费 technical `EvidenceBundle` 和 concrete `PromptRunBinding`，allowlist 限定 `screen_snapshot`/`screen_pipeline_snapshot` + `screening`、`factor_evaluation` + `factor_evaluation`、`factor_cache_manifest` + `dataset_lineage`，并要求 Evidence metadata `llm_recompute_allowed=false`，拒绝 formal backtest/risk/audit/API/UI lineage evidence。结构化输出必须使用 current bundle citation graph，numeric Claim 必须为 `deterministic_evidence` 且 citation 包含 unit/formula_version，claim 与 citation 的 value/unit/formula/dataset/run/artifact 保持一致，citation 与 Evidence 的 dataset/run/stage/artifact 保持一致；兼容层输出 legacy DSA `technical` opinion、top-level dashboard fields 和 nested `data_perspective`。本任务不调用 Provider/LLM、不执行 DSA tools、不启动 Worker/Qlib/Citation Validator/报告渲染或正式组合回测推广 | [technical-agent-evidence-adapter.md](./technical-agent-evidence-adapter.md); [technical_agent.py](../src/serenity_alpha_lab/application/technical_agent.py); [test_technical_agent_evidence_adapter.py](../tests/application/test_technical_agent_evidence_adapter.py) | SAL-P5-008,SAL-P5-009,SAL-P5-012,SAL-P5-013,SAL-P5-015 | G5 |
| DEC-095 | 2026-07-27 | Intel Agent 证据适配口径 | 采用 `application.intel_agent.EvidenceScopedIntelAgent` 冻结 `research.agent.intel@1.0.0` 离线 Intel Agent adapter 边界；adapter 只消费 intel `EvidenceBundle` 和 concrete `PromptRunBinding`，allowlist 限定 `unstructured_source` + `market_intelligence`，并要求 Evidence metadata `source_trust` 和 `llm_recompute_allowed=false`。Prompt payload 保留 source trust prompt-safe record、event/published/observed/available timestamps、URL/body hashes、trust、freshness、corroboration 和 exclusion reasons；duplicate source hash 与 malicious instruction source 被排除，stale/low-trust source 可作为 context 但不能支撑 strong event。结构化输出必须使用 current bundle citation graph，Intel Agent 不得新增 `numeric_metric` Claim；兼容层输出 legacy DSA `intel` opinion、news/intel dashboard fields、sentiment、key_events 和 source_quality。本任务不抓取新闻/搜索/社交内容、不调用 Provider/LLM、不执行 DSA tools、不启动 Worker/Qlib/Citation Validator/报告渲染或正式组合回测推广 | [intel-agent-evidence-adapter.md](./intel-agent-evidence-adapter.md); [intel_agent.py](../src/serenity_alpha_lab/application/intel_agent.py); [test_intel_agent_evidence_adapter.py](../tests/application/test_intel_agent_evidence_adapter.py) | SAL-P5-009,SAL-P5-010,SAL-P5-012,SAL-P5-013,SAL-P5-015 | G5 |
| DEC-096 | 2026-07-28 | Risk/Portfolio Agent 证据适配口径 | 采用 `application.risk_portfolio_agent.EvidenceScopedRiskPortfolioAgent` 冻结 `research.agent.risk_portfolio@1.0.0` 离线 Risk/Portfolio Agent adapter 边界；adapter 只消费 `risk_portfolio` EvidenceBundle 和 concrete PromptRunBinding，allowlist 限定 `risk_policy_result`、`backtest_bias_audit`、`backtest_performance_metrics`、`backtest_run_summary`、`backtest_artifact_bundle` 和 `formal_backtest_api_record`，并要求 Evidence metadata `llm_recompute_allowed=false`。Prompt payload 汇总 deterministic hard gate status，`block`、`not_evaluable`、`eligible_for_ranking=false` 和 `agent_strong_conclusion_allowed=false` 不允许被 Agent 输出升级；结构化输出必须使用 current bundle citation graph，numeric Claim 必须为 `deterministic_evidence` 且 citation 与 Evidence 的 dataset/run/stage/artifact/formula 及 claim value/unit/formula 保持一致；兼容层输出 legacy DSA `risk_portfolio` opinion/dashboard fields。本任务不调用 Provider/LLM、不执行 DSA tools、不启动 Worker/Qlib/Citation Validator/报告渲染或正式组合回测推广 | [risk-portfolio-agent-evidence-adapter.md](./risk-portfolio-agent-evidence-adapter.md); [risk_portfolio_agent.py](../src/serenity_alpha_lab/application/risk_portfolio_agent.py); [test_risk_portfolio_agent_evidence_adapter.py](../tests/application/test_risk_portfolio_agent_evidence_adapter.py) | SAL-P5-010,SAL-P5-011,SAL-P5-012,SAL-P5-013,SAL-P5-015 | G5 |
| DEC-097 | 2026-07-28 | Decision Agent 多空反证与最终综合口径 | 采用 `application.decision_agent.EvidenceScopedDecisionAgent` 冻结 `research.agent.decision@1.0.0` 离线 Decision Agent 综合边界；adapter 只消费已验证的 Technical、Intel、Risk/Portfolio result、decision EvidenceBundle 和 concrete PromptRunBinding，记录 role result hashes、prior-role citation evidence ids、allowed evidence ids/hashes、preserved risk gate summary 和 forbidden runtime actions。结构化输出必须包含 distinct bull/bear cases、disagreement summary、invalidation conditions 和 final recommendation；所有 citations 必须在当前 Decision EvidenceBundle 中，且 underlying evidence 必须已被前序 role result 引用，case `source_roles` 必须匹配前序引用角色；numeric Claim 只能使用 `deterministic_evidence` 且与 citation value/unit/formula/dataset/run/stage/artifact 完全一致；final recommendation 不得升级 Risk/Portfolio `block` / `not_evaluable`、ranking ineligible 或 strong-conclusion hard gate。本任务不调用 Provider/LLM、不执行 DSA tools、不读 Evidence body、不写 Evidence Store、不启动 Worker/Qlib、model routing/cache/budget、Citation Validator、报告渲染或正式组合回测推广 | [decision-agent-counterargument-synthesis.md](./decision-agent-counterargument-synthesis.md); [decision_agent.py](../src/serenity_alpha_lab/application/decision_agent.py); [test_decision_agent_counterargument_synthesis.py](../tests/application/test_decision_agent_counterargument_synthesis.py) | SAL-P5-011,SAL-P5-012,SAL-P5-013,SAL-P5-015 | G5 |
| DEC-098 | 2026-07-28 | 模型路由、缓存与预算口径 | 采用 `application.model_routing.ModelInvocationPlanner` 冻结 `research.model_routing@1.0.0` 离线模型调用规划边界；planner 只消费 `EvidenceBundle`、`PromptRunBinding`、版本化 `ModelInvocationParameters`、候选 `ModelRouteCandidate`、离线 `ModelPriceTable`、调用方提供的 budget usage snapshot 和成功 receipt metadata。`ModelInvocationCacheKey` 必须包含 EvidenceBundle hash、PromptRunBinding hash、output schema hash、provider/model/version、model capability hash、parameter version/hash、route version 和 price version；cache hit 不消耗预算；预算同时检查 invocation/run/day 三层，无法 fallback 时返回 `budget_exhausted`；限流 route 显式 fallback 或返回 `rate_limited`。本任务不导入 LiteLLM、不调用 Provider/LLM、不写 AgentStageStore receipt、不启动 Worker/Qlib、Citation Validator、报告渲染、生产调度或正式组合回测推广 | [model-routing-cache-budget.md](./model-routing-cache-budget.md); [model_routing.py](../src/serenity_alpha_lab/application/model_routing.py); [test_model_routing_cache_budget.py](../tests/application/test_model_routing_cache_budget.py) | SAL-P5-012,SAL-P5-013,SAL-P5-014,SAL-P5-015,SAL-P6-013 | G5 |
| DEC-099 | 2026-07-28 | Citation Validator 口径 | 采用 `evidence.citation_validator.CitationValidator` 冻结 `research.citation_validator@1.0.0` 离线引用校验边界；validator 只消费已构造的 `ResearchReport`、`ResearchClaim`、`ReportCitation` 和 `EvidenceRecord` schema objects，强制 `numeric_metric`、`temporal_fact`、`risk_gate`、`lineage_fact` 引用，校验 citation evidence 存在、`available_at <= decision_time`、claim/citation/evidence 的 value、unit、formula_version、dataset_versions、run_id、stage_id 和 artifact_hash 一致性。一次 caller-supplied repair attempt 后仍失败的 Claim 必须从 validated report 删除并将 report 降级，失败 Claim 不得保留 `verified`。本任务不读 Evidence body、不调用 Provider/LLM、不执行 Agent stage、不写 Evidence Store、不启动 Worker/Qlib、报告渲染、生产调度或正式组合回测推广 | [citation-validator.md](./citation-validator.md); [citation_validator.py](../src/serenity_alpha_lab/evidence/citation_validator.py); [test_citation_validator.py](../tests/evidence/test_citation_validator.py) | SAL-P5-013,SAL-P5-014,SAL-P5-015,SAL-P5-016,SAL-P5-017 | G5 |
| DEC-100 | 2026-07-28 | Agent 工具安全口径 | 采用 `application.agent_tool_security.AgentToolSecurityGuard` 冻结 `research.agent_tool_security@1.0.0` 离线工具授权边界；guard 只消费 concrete `PromptRunBinding`、调用方 stage tool allowlist 和调用方参数，默认拒绝未绑定或未在 stage allowlist 中的工具，普通输入失败返回结构化 issue 而不执行工具。运行时必须校验 Prompt Registry tool JSON-Schema subset、阻止 shell/trading/brokerage/database/filesystem write scope、默认禁止 HTTP/本地/私网/metadata/IP SSRF URL，并要求 metadata `allowed_url_hosts` 中声明的主机；参数中的外部 prompt/tool/system/developer instruction、secret/token/API key 泄露请求和 shell/admin/root escalation hints 必须返回 `prompt_injection` deny。所有 decision 均 `would_execute=false`，本任务不调用 Provider/LLM、不执行工具、不读 Evidence body、不写 Evidence Store、不启动 Worker/Qlib、Citation Validator repair、报告渲染、生产调度或正式组合回测推广 | [agent-tool-security.md](./agent-tool-security.md); [agent_tool_security.py](../src/serenity_alpha_lab/application/agent_tool_security.py); [test_agent_tool_security.py](../tests/application/test_agent_tool_security.py) | SAL-P5-014,SAL-P5-015,SAL-P5-017,SAL-P6-013 | G5 |
| DEC-101 | 2026-07-29 | 可信 ResearchReport Renderer 口径 | 采用 `evidence.report_renderer.TrustedResearchReportRenderer` 冻结 `research.report_renderer@1.0.0` 离线可信报告渲染边界；renderer 只消费结构化 `ResearchReport` 和可选 `CitationValidationResult`，先以 `CitationValidator` 校验/降级，再输出 `authority=canonical_json` 的权威 JSON envelope、deterministic `authoritative_json_hash`、模板版本和从该 envelope 派生的 Markdown/HTML。Markdown/HTML 必须标记为 `derived_from_authoritative_json`，不得反向作为权威数据源；报告必须显示 report level、as-of、Dataset versions、model metadata、prompt/cost context、risk summary、disclaimer、claims、citations、evidence lineage 和 validation issues。本任务不实现 UI/通知/Outbox、不调用 Provider/LLM、不执行 Agent/tool、不读 Evidence body、不写 Evidence Store、不启动 Worker/Qlib、Citation repair、生产调度或正式组合回测推广 | [trusted-research-report-renderer.md](./trusted-research-report-renderer.md); [report_renderer.py](../src/serenity_alpha_lab/evidence/report_renderer.py); [test_report_renderer.py](../tests/evidence/test_report_renderer.py) | SAL-P5-015,SAL-P5-016,SAL-P5-017 | G5 |
| DEC-102 | 2026-07-29 | 引用 UI 与通知 Outbox 口径 | 采用 `application.report_delivery.ResearchReportPagePresenter` 冻结 `research.report_delivery_ui@1.0.0` 离线报告页面 payload 边界，页面权威来源只能是 `RenderedResearchReport.trusted_report.authoritative_json`，Markdown/HTML 仅作为 `derived_from_authoritative_json` 展示；payload 必须保留 authoritative/rendering hash、report/model/cost/risk/disclaimer/Dataset context，并把每个 Claim 展开到 Citation、Evidence、source URI 和 artifact id/hash。采用 `repositories.notification_outbox.NotificationOutboxStore` 冻结 `research.notification_outbox@1.0.0` 持久化 metadata 边界，按 `(tenant_id, channel, dedupe_key)` 幂等，支持 pending/sending/sent/dead_letter lease/retry 状态但不执行 sender。DSA Web 通过 `DSA-PATCH-007` 新增 GET-only `/research-reports/:reportId` 页面，不注册后端 route、不调用通知 provider、不启动 Provider/LLM/Worker/Qlib runtime 或 Gate G5 promotion | [research-report-delivery-ui-outbox.md](./research-report-delivery-ui-outbox.md); [report_delivery.py](../src/serenity_alpha_lab/application/report_delivery.py); [notification_outbox.py](../src/serenity_alpha_lab/repositories/notification_outbox.py); [0007-add-research-report-delivery-ui.patch](../patches/dsa/v3.26.1/0007-add-research-report-delivery-ui.patch) | SAL-P5-016,SAL-P5-017,SAL-P5-018 | G5 |
| DEC-103 | 2026-07-30 | Agent 金标回归评测口径 | 采用 `application.agent_evaluation` 冻结 `research.agent_evaluation@1.0.0` 离线 Agent evaluation 边界；默认 golden catalog 必须含 50+ 案例并覆盖正常、缺失数据、财务异常、重大事件、观点冲突、恶意内容和多市场样本；评测只消费结构化 `ResearchClaim` 与 `ReportCitation`，由 deterministic stub、scorer 和 regression comparison 计算引用准确率、无依据数值率、schema 成功率、安全核心集和模型/Prompt 版本差异。默认门槛为 citation accuracy `>=95%`、unsupported numeric rate `<1%`、schema success `100%`、safety core 全通过。本任务不调用 Provider/LLM、不执行 Agent/tool、不读 Evidence body、不写 Evidence Store、不启动 Worker/Qlib、报告发布、生产调度、Gate G5 或正式组合回测推广 | [agent-golden-regression-evaluation.md](./agent-golden-regression-evaluation.md); [agent_evaluation.py](../src/serenity_alpha_lab/application/agent_evaluation.py); [test_agent_golden_regression_evaluation.py](../tests/application/test_agent_golden_regression_evaluation.py) | SAL-P5-017,SAL-P5-018 | G5 |
| DEC-104 | 2026-07-30 | Gate G5 可信研究评审 | `GO with accepted risks`：批准 P5 离线可信研究链进入 P6 RC hardening 输入；Evidence/Claim/Report、Evidence Store、EvidenceBundle、Source Trust、Quant Evidence Adapter、Prompt Registry、Agent Stage Store、Technical/Intel/RiskPortfolio/Decision adapters、Model Routing、Citation Validator、Agent Tool Security、Trusted ResearchReport Renderer、Research Report Delivery UI、Notification Outbox 和 Agent Golden regression 已满足 G5 条件。Gate G5 不批准真实 Provider/LLM、Agent/tool execution、Worker loop、Qlib runtime、notification sender、production scheduler、formal backtest promotion 或 P6 实现提前启动 | [gate-g5-trusted-research-review.md](./gate-g5-trusted-research-review.md); [test_gate_g5_trusted_research_review.py](../tests/gates/test_gate_g5_trusted_research_review.py); [2026-07-30-gate-g5-trusted-research-review.md](./superpowers/plans/2026-07-30-gate-g5-trusted-research-review.md) | SAL-P5-018,SAL-P6-001 | G6 |
| DEC-105 | 2026-07-30 | Auth/RBAC 身份与权限模型口径 | 采用 `application.auth_rbac` 冻结 `security.auth_rbac@1.0.0` framework-neutral 认证与 RBAC 契约；desktop 使用 local owner 保持本地易用，standalone 使用本地 admin/researcher/viewer/auditor/service-worker，team mode 要求预验证 OIDC/外部主体并显式分离 data/run/config/admin 权限。OIDC provider 只声明 issuer/client/audience/secret-present 状态，claim mapping 只消费预验证 claims，不验证 JWT、不抓取 JWK、不导入 Authlib/JWT/network SDK。API authorization catalog 覆盖 auth/config/user、quant definition、screen/backtest run、artifact download、evidence/report、notification outbox metadata 和 audit surfaces；本任务不实现对象级 signed URL、Secret Manager、Worker loop、真实 Provider/LLM、Qlib runtime、notification sender、production scheduler 或 formal backtest promotion | [auth-rbac.md](./auth-rbac.md); [auth_rbac.py](../src/serenity_alpha_lab/application/auth_rbac.py); [test_auth_rbac.py](../tests/application/test_auth_rbac.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-30-auth-rbac.md](./superpowers/plans/2026-07-30-auth-rbac.md) | SAL-P6-001,SAL-P6-002 | G6 |
| DEC-106 | 2026-07-30 | 资源与 Artifact 授权口径 | 采用 `application.resource_authorization.ResourceAuthorizationPolicy` 冻结 `security.resource_artifact_authorization@1.0.0` framework-neutral 对象级授权边界；所有 Run、Definition、Evidence、Report 和 Artifact download 先通过 SAL-P6-001 RBAC，再校验 tenant/team/private owner scope。Artifact download 还必须绑定 parent resource read access，并通过 `SignedArtifactUrlIssuer` 签发短时 HMAC URL，审计记录只保留 signature hash 不保留 raw signature/key。`WorkerResourceGrant` 只允许 service_worker 在指定 task/run/artifact ids 上使用 `run:read` 和 `artifact:download`，不得使用 wildcard。本任务不接入 Secret Manager/对象存储/FastAPI middleware，不启动 Worker loop、真实 Provider/LLM、Qlib runtime、notification sender、production scheduler 或 formal backtest promotion | [resource-artifact-authorization.md](./resource-artifact-authorization.md); [resource_authorization.py](../src/serenity_alpha_lab/application/resource_authorization.py); [test_resource_authorization.py](../tests/application/test_resource_authorization.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py) | SAL-P6-002,SAL-P6-003 | G6 |
| DEC-107 | 2026-07-31 | 密钥引用、配置诊断与审计口径 | 采用 `application.config_profiles` 扩展 `SAL-P1-014` profile facade，冻结 metadata-only `SecretReference`、`SecretRotationPlan`、`ConfigAuditRecord` 和 `config_api_diagnostics()` 口径；Secret Manager/OS Keychain 只以 `secretmanager://` / `keychain://` 引用、last-four、版本和 hash 表达，API 诊断对 secret 字段只返回 configured/backend/last_four/source/sensitive，不返回 `value` 或明文。轮换和审计只记录 dry-run/intent、reference hash 和 sanitized metadata。本任务不导入真实 Keychain/Secret Manager/cloud SDK，不注册 FastAPI config route、不写 `.env`/数据库/备份、不启动 Provider/LLM/Worker/Qlib/sender/scheduler/release packaging 或 formal backtest promotion | [secret-config-hardening.md](./secret-config-hardening.md); [config_profiles.py](../src/serenity_alpha_lab/application/config_profiles.py); [test_config_profiles.py](../tests/application/test_config_profiles.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-31-secret-config-hardening.md](./superpowers/plans/2026-07-31-secret-config-hardening.md) | SAL-P6-003,SAL-P6-004,SAL-P6-009,SAL-P6-016 | G6 |
| DEC-108 | 2026-07-31 | 输入、抓取与报告渲染加固口径 | 采用 `application.input_fetch_security` 冻结 `security.input_fetch_report_hardening@1.0.0` framework-neutral 输入与显示安全边界；URL/fetch policy 只基于调用方提供的 URL、redirect、resolved IP 和 response metadata 决策，默认要求 HTTPS、public IP、host allowlist、redirect/size/content-type limits。Upload policy 只基于 metadata/sample bytes 做 first-line reject；Report policy 校验 `RenderedResearchReport.html` active content 和 source links，`ResearchReportPagePresenter` 保持 canonical JSON authority、过滤 unsafe source links 并附加 security headers。本任务不导入 requests/httpx/FastAPI/Provider/LLM/Worker/Qlib/SQLAlchemy runtime，不执行 fetch/upload/parser/sender/scheduler/release packaging 或 formal backtest promotion | [input-fetch-report-hardening.md](./input-fetch-report-hardening.md); [input_fetch_security.py](../src/serenity_alpha_lab/application/input_fetch_security.py); [report_delivery.py](../src/serenity_alpha_lab/application/report_delivery.py); [test_input_fetch_report_security.py](../tests/application/test_input_fetch_report_security.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-31-input-fetch-report-hardening.md](./superpowers/plans/2026-07-31-input-fetch-report-hardening.md) | SAL-P6-004,SAL-P6-005,SAL-P6-013 | G6 |
...
| AEV-092 | SAL-P5-004 | 来源信任、非结构化清洗、URL/body hash、time conflict 和 prompt-safe guard 验证记录 | [source-trust-unstructured-cleaning.md](./source-trust-unstructured-cleaning.md); [source_trust.py](../src/serenity_alpha_lab/evidence/source_trust.py); [test_source_trust_cleaning.py](../tests/evidence/test_source_trust_cleaning.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py) | Red target failed with missing `serenity_alpha_lab.evidence.source_trust` (`1 error`); Green focused target `5 passed`; related SourceTrust/EvidenceStore/EvidenceBundle/Architecture suite `33 passed`; full pytest `423 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers official source authoritative trust、canonical URL/tracking removal、URL/raw body/cleaned body SHA-256、low-trust corroboration and strong-claim rejection、external prompt/tool instruction removal from prompt-safe record、time conflict warning and timezone-aware timestamp guard; no Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA runtime import boundary; no Evidence Agent、Quant Evidence Adapter、Citation Validator、report renderer、production scheduler 或 formal backtest promotion | AI/SEC | 2026-07-26 |
| AEV-093 | SAL-P5-005 | Quant Evidence Adapter、Screen/Factor/Backtest/Risk DTO 到 EvidenceRecord 与 deterministic citation 映射验证记录 | [quant-evidence-adapter.md](./quant-evidence-adapter.md); [quant_adapter.py](../src/serenity_alpha_lab/evidence/quant_adapter.py); [test_quant_evidence_adapter.py](../tests/evidence/test_quant_evidence_adapter.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py) | Red target failed with missing `serenity_alpha_lab.evidence.quant_adapter` (`1 error`); audit path Red failed with missing `from_backtest_bias_audit_report`; Green focused target `3 passed`; related QuantEvidenceAdapter/EvidenceSchema/SourceTrust/EvidenceStore/EvidenceBundle/Architecture suite `37 passed`; full pytest `427 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers Screen/Factor vs formal backtest scope separation、ArtifactManifest canonical body hash match、dataset_versions/artifact_hash/run/stage/trace retention、numeric citation unit/formula_version and `llm_recompute_allowed=false`; no Evidence Store writes、EvidenceBundle、Agent、Provider/LLM、Worker、Qlib runtime、Citation Validator、report renderer 或 formal backtest promotion | AI/QE | 2026-07-26 |
| AEV-094 | SAL-P5-006 | Prompt/Output Schema Registry、published Prompt immutability、schema compatibility、tool/model declarations 和 run binding 验证记录 | [prompt-output-schema-registry.md](./prompt-output-schema-registry.md); [prompt_registry.py](../src/serenity_alpha_lab/evidence/prompt_registry.py); [test_prompt_schema_registry.py](../tests/evidence/test_prompt_schema_registry.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py) | Red target failed with missing `serenity_alpha_lab.evidence.prompt_registry` (`1 error`); Green focused target `4 passed`; prompt registry architecture guard `1 passed`; related PromptRegistry/EvidenceSchema/SourceTrust/QuantAdapter/EvidenceBundle/Architecture suite `38 passed`; full pytest `432 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; contract covers four published role prompts、read-only EvidenceBundle tool declaration、registry-only model capability、published prompt immutability、minor output schema optional-only compatibility、forbidden shell/trading/db-write tool categories、run binding concrete version/hash capture and no Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA runtime import boundary; no Evidence Agent、Citation Validator、report renderer、production scheduler 或 formal backtest promotion | AI/SEC | 2026-07-27 |
| AEV-095 | SAL-P5-007 | Agent Stage 持久化、PromptRunBinding checkpoint、resume plan、model-call receipt 幂等和 failure policy 验证记录 | [agent-stage-persistence.md](./agent-stage-persistence.md); [agent_stage_store.py](../src/serenity_alpha_lab/repositories/agent_stage_store.py); [test_agent_stage_store.py](../tests/repositories/test_agent_stage_store.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py) | Red target failed with missing `serenity_alpha_lab.repositories.agent_stage_store` (`1 error`); Green focused target `4 passed`; agent stage architecture guard `1 passed`; related AgentStageStore/PersistentTaskBackend/TaskEventStream/RunLifecycle/PromptRegistry/ResearchOrchestrator/Architecture suite `54 passed`; full pytest `437 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers deterministic `stage_id`、concrete prompt version/hash guard、stage/binding match、persistent checkpoint after restart、successful model-call receipt idempotency and conflict detection、`reuse_model_call` resume action、cancel propagation、`degrade`/`skip`/`fail_run` policy and no Provider/LLM/Worker/Qlib/FastAPI/DSA Agent runtime import boundary; no Evidence Agent execution、model budget/cache、Citation Validator、report renderer、production scheduler 或 formal backtest promotion | AI/BE | 2026-07-27 |
| AEV-096 | SAL-P5-008 | Technical Agent Evidence Adapter、technical/factor evidence allowlist、numeric Claim citation guard 和 DSA compatibility mapping 验证记录 | [technical-agent-evidence-adapter.md](./technical-agent-evidence-adapter.md); [technical_agent.py](../src/serenity_alpha_lab/application/technical_agent.py); [test_technical_agent_evidence_adapter.py](../tests/application/test_technical_agent_evidence_adapter.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-27-technical-agent-evidence-adapter.md](./superpowers/plans/2026-07-27-technical-agent-evidence-adapter.md) | Red target failed with missing `serenity_alpha_lab.application.technical_agent` (`1 error`); review regression Red target `3 failed, 4 passed` for missing scope/recompute guard、numeric citation mismatch guard and DSA `data_perspective`; Green focused target `7 passed`; TechnicalAgent architecture guard `1 passed`; related TechnicalAgent/EvidenceBundle/QuantEvidenceAdapter/PromptRegistry/AgentStageStore/Architecture suite `41 passed`; full pytest `445 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers technical PromptRunBinding + EvidenceBundle payload hash、PromptRunBinding run/stage context match、kind+scope+`llm_recompute_allowed=false` allowlist、numeric deterministic citation value/unit/formula/dataset/run/artifact guard、citation evidence dataset/run/stage/artifact validation、DSA-compatible opinion/dashboard and nested `data_perspective` fields and no Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA Agent runtime import boundary; no Intel/Risk/Decision Agent、model budget/cache、Citation Validator、report renderer、production scheduler 或 formal backtest promotion | AI/QE | 2026-07-27 |
| AEV-097 | SAL-P5-009 | Intel Agent Evidence Adapter、source-trust-backed unstructured evidence、event/published/observed/available time separation、duplicate/stale/malicious handling 和 DSA compatibility mapping 验证记录 | [intel-agent-evidence-adapter.md](./intel-agent-evidence-adapter.md); [intel_agent.py](../src/serenity_alpha_lab/application/intel_agent.py); [test_intel_agent_evidence_adapter.py](../tests/application/test_intel_agent_evidence_adapter.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-27-intel-agent-evidence-adapter.md](./superpowers/plans/2026-07-27-intel-agent-evidence-adapter.md) | Red target failed with missing `serenity_alpha_lab.application.intel_agent` (`1 error`); Green focused target `4 passed`; IntelAgent architecture guard `1 passed`; related IntelAgent/EvidenceBundle/SourceTrust/PromptRegistry/EvidenceSchema/AgentStageStore/Architecture suite `47 passed`; full pytest `451 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; disposable DSA worktree sequentially applied patches `0001..0006`; `git diff --check` PASS; contract covers `unstructured_source` / `market_intelligence` Evidence representation、Intel PromptRunBinding + EvidenceBundle payload hash、PromptRunBinding run/stage context match、source trust metadata guard、duplicate source hash exclusion、malicious instruction exclusion、stale/low-trust strong-claim guard、current-bundle citation graph validation、Intel no numeric_metric claims and DSA-compatible intel/news/sentiment/source_quality fields; no Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA Agent runtime import boundary; no Risk/Decision Agent、model budget/cache、Citation Validator、report renderer、production scheduler 或 formal backtest promotion | AI/QE | 2026-07-27 |
| AEV-098 | SAL-P5-010 | Risk/Portfolio Agent Evidence Adapter、formal risk/portfolio evidence allowlist、hard gate preservation、numeric Claim citation guard 和 DSA compatibility mapping 验证记录 | [risk-portfolio-agent-evidence-adapter.md](./risk-portfolio-agent-evidence-adapter.md); [risk_portfolio_agent.py](../src/serenity_alpha_lab/application/risk_portfolio_agent.py); [test_risk_portfolio_agent_evidence_adapter.py](../tests/application/test_risk_portfolio_agent_evidence_adapter.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-28-risk-portfolio-agent-evidence-adapter.md](./superpowers/plans/2026-07-28-risk-portfolio-agent-evidence-adapter.md) | Red target failed with missing `serenity_alpha_lab.application.risk_portfolio_agent` (`1 error`); Green focused target `4 passed`; RiskPortfolio architecture guard `1 passed`; related RiskPortfolioAgent/EvidenceBundle/QuantEvidenceAdapter/PromptRegistry/AgentStageStore/Architecture suite `41 passed`; full pytest `456 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers risk_portfolio PromptRunBinding + EvidenceBundle payload hash、formal portfolio evidence allowlist、`llm_recompute_allowed=false` guard、`block`/`not_evaluable` hard gate preservation、ranking/strong-conclusion guard metadata retention、numeric deterministic citation value/unit/formula/dataset/run/stage/artifact guard、citation evidence dataset/run/stage/artifact validation、DSA-compatible risk/portfolio opinion/dashboard fields and no Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA Agent runtime import boundary; no Decision Agent、model budget/cache、Citation Validator、report renderer、production scheduler 或 formal backtest promotion | AI/QE | 2026-07-28 |
| AEV-099 | SAL-P5-011 | Decision Agent 多空反证、最终综合、前序引用 guard、Risk hard gate preservation 和 DSA compatibility mapping 验证记录 | [decision-agent-counterargument-synthesis.md](./decision-agent-counterargument-synthesis.md); [decision_agent.py](../src/serenity_alpha_lab/application/decision_agent.py); [test_decision_agent_counterargument_synthesis.py](../tests/application/test_decision_agent_counterargument_synthesis.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-28-decision-counterargument-synthesis.md](./superpowers/plans/2026-07-28-decision-counterargument-synthesis.md) | Red target failed with missing `serenity_alpha_lab.application.decision_agent` (`1 error`); Green focused target `4 passed`; Decision architecture guard `1 passed`; related Decision/Technical/Intel/RiskPortfolio/EvidenceBundle/PromptRegistry/AgentStageStore/Architecture suite `54 passed`; full pytest `461 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers decision PromptRunBinding + EvidenceBundle payload hash、prior role result hashes、prior-role citation evidence id guard、distinct bull/bear cases、disagreement/invalidation citations、no new facts outside current Decision bundle and prior role outputs、case `source_roles` alignment、numeric deterministic claim value/unit/formula/dataset/run/stage/artifact guard、Risk/Portfolio `block`/`not_evaluable`/ranking/strong-conclusion preservation and no Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA Agent runtime import boundary; no model routing/cache/budget、Citation Validator、report renderer、production scheduler 或 formal backtest promotion | AI/QE | 2026-07-28 |
| AEV-100 | SAL-P5-012 | Model routing/cache/budget、精确 cache key、三级预算、限流 fallback 和 offline boundary 验证记录 | [model-routing-cache-budget.md](./model-routing-cache-budget.md); [model_routing.py](../src/serenity_alpha_lab/application/model_routing.py); [test_model_routing_cache_budget.py](../tests/application/test_model_routing_cache_budget.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-28-model-routing-cache-budget.md](./superpowers/plans/2026-07-28-model-routing-cache-budget.md) | Red target failed with missing `serenity_alpha_lab.application.model_routing` (`1 error`); Green focused target `4 passed`; ModelRouting architecture guard `1 passed`; related ModelRouting/PromptRegistry/EvidenceBundle/AgentStageStore/Architecture suite `40 passed`; full pytest `466 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers EvidenceBundle/PromptRunBinding/model/parameter/route/price-version cache key、successful receipt reuse with zero budget spend、invocation/run/day budget-exhausted partial result、explicit lower-priority fallback for budget/rate-limit degradation and no LiteLLM/Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA runtime import boundary; no Citation Validator、report renderer、production scheduler 或 formal backtest promotion | AI/BE | 2026-07-28 |
| AEV-101 | SAL-P5-013 | Citation Validator、mandatory citation、claim/citation/evidence lineage consistency、repair deletion 和 offline boundary 验证记录 | [citation-validator.md](./citation-validator.md); [citation_validator.py](../src/serenity_alpha_lab/evidence/citation_validator.py); [test_citation_validator.py](../tests/evidence/test_citation_validator.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-28-citation-validator.md](./superpowers/plans/2026-07-28-citation-validator.md) | Red target failed with missing `serenity_alpha_lab.evidence.citation_validator` (`1 error`); initial Green focused target `3 passed`; local review regression Red target caught citation/evidence lineage issue not attached to consuming claims (`1 failed, 4 passed`); final Green focused target `7 passed`, including missing cited evidence and future evidence invalid graph regressions; CitationValidator architecture guard `1 passed`; related CitationValidator/EvidenceSchema/QuantEvidenceAdapter/Technical/Intel/RiskPortfolio/Decision/Architecture suite `59 passed`; full pytest `474 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; contract covers mandatory citation for numeric/date/risk/lineage claims、citation evidence existence、decision-time guard、claim/citation/evidence value/unit/formula/dataset/run/stage/artifact consistency、failed claim downgrade、one-attempt repair deletion and no Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA runtime import boundary; no Evidence body read、Evidence Store write、Agent stage、report renderer、production scheduler 或 formal backtest promotion | AI/QE | 2026-07-28 |
| AEV-102 | SAL-P5-014 | Agent Tool Security、默认 deny、参数 Schema、SSRF/URL allowlist 和 prompt-injection guard 验证记录 | [agent-tool-security.md](./agent-tool-security.md); [agent_tool_security.py](../src/serenity_alpha_lab/application/agent_tool_security.py); [test_agent_tool_security.py](../tests/application/test_agent_tool_security.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-28-agent-tool-security.md](./superpowers/plans/2026-07-28-agent-tool-security.md) | Red target failed with missing `serenity_alpha_lab.application.agent_tool_security` (`1 error`); Green focused target `4 passed`; AgentToolSecurity architecture guard `1 passed`; related AgentToolSecurity/PromptRegistry/SourceTrust/ModelRouting/AgentStageStore/Architecture suite `47 passed`; full pytest `479 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers PromptRunBinding-bound tool checks、stage tool allowlist、no shell/trading/database/filesystem write scope、JSON-Schema subset validation、safe_arguments empty on deny、`would_execute=false` for every decision、HTTP/private/loopback/link-local/metadata SSRF URL denial、metadata host allowlist and prompt-injection argument denial; no tool execution、Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA runtime import boundary; no Evidence body read、Evidence Store write、Citation Validator repair、report renderer、production scheduler 或 formal backtest promotion | AI/SEC | 2026-07-28 |
| AEV-103 | SAL-P5-015 | Trusted ResearchReport Renderer、权威 JSON envelope、Markdown/HTML 派生渲染、降级报告展示和 offline boundary 验证记录 | [trusted-research-report-renderer.md](./trusted-research-report-renderer.md); [report_renderer.py](../src/serenity_alpha_lab/evidence/report_renderer.py); [test_report_renderer.py](../tests/evidence/test_report_renderer.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-29-trusted-research-report-renderer.md](./superpowers/plans/2026-07-29-trusted-research-report-renderer.md) | Red target failed with missing `serenity_alpha_lab.evidence.report_renderer` (`1 error`); Green focused target `4 passed`; ReportRenderer architecture guard `1 passed`; related P5 renderer suite `73 passed`; full pytest `484 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; contract covers canonical JSON authority、deterministic authoritative hash、Markdown/HTML `derived_from_authoritative_json`、verified/partial/insufficient-evidence display、model/cost/risk/disclaimer metadata、HTML escaping and no Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA runtime import boundary; no UI/notification/Outbox、Evidence body read、Evidence Store write、Agent/tool execution、Citation repair、production scheduler 或 formal backtest promotion | AI/FE | 2026-07-29 |
| AEV-104 | SAL-P5-016 | ResearchReport 引用 UI payload、通知 Outbox metadata、DSA Web report page 和 offline/runtime boundary 验证记录 | [research-report-delivery-ui-outbox.md](./research-report-delivery-ui-outbox.md); [report_delivery.py](../src/serenity_alpha_lab/application/report_delivery.py); [notification_outbox.py](../src/serenity_alpha_lab/repositories/notification_outbox.py); [test_report_delivery_ui.py](../tests/application/test_report_delivery_ui.py); [test_notification_outbox.py](../tests/repositories/test_notification_outbox.py); [0007-add-research-report-delivery-ui.patch](../patches/dsa/v3.26.1/0007-add-research-report-delivery-ui.patch); [2026-07-29-citation-ui-notification-outbox.md](./superpowers/plans/2026-07-29-citation-ui-notification-outbox.md) | Red target failed with missing `serenity_alpha_lab.application.report_delivery` and `serenity_alpha_lab.repositories.notification_outbox`; focused Python/architecture target `6 passed`; DSA Web focused target `4 passed files / 26 passed tests`; DSA Web lint/build PASS; related P5/report/outbox suite `79 passed`; full pytest `490 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; clean temp DSA worktree sequentially applied `0001..0007`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; contract covers canonical JSON-only page authority、claim/citation/evidence/source/artifact expansion、GET-only report/notification routes、Outbox tenant/channel/dedupe idempotency、lease/retry/sent/dead_letter metadata and no Provider/LLM/Worker/Qlib/FastAPI/DSA sender runtime boundary; no Agent evaluation、Gate G5、notification sender、Evidence Store write、Citation repair、production scheduler 或 formal backtest promotion | FE/BE | 2026-07-29 |
| AEV-105 | SAL-P5-017 | Agent 金标与回归评测、56-case catalog、离线 stub、scorer、模型/Prompt regression comparison 和 offline boundary 验证记录 | [agent-golden-regression-evaluation.md](./agent-golden-regression-evaluation.md); [agent_evaluation.py](../src/serenity_alpha_lab/application/agent_evaluation.py); [test_agent_golden_regression_evaluation.py](../tests/application/test_agent_golden_regression_evaluation.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-30-agent-golden-regression-evaluation.md](./superpowers/plans/2026-07-30-agent-golden-regression-evaluation.md) | Red target failed with missing `serenity_alpha_lab.application.agent_evaluation` (`1 error`); focused target `5 passed`; related P5 suite `76 passed`; full pytest `495 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; catalog has `56` cases, `8` per category across normal/missing_data/financial_anomaly/major_event/viewpoint_conflict/malicious_content/multi_market; baseline metrics are citation accuracy `1.0`, unsupported numeric rate `0.0`, schema success `1.0`, safety core passed; regression tests catch `missing_expected_citation`、`unsupported_numeric_claim` and `safety_check_failed`; no Provider/LLM/Worker/Qlib/FastAPI/SQLAlchemy/DSA Agent runtime import boundary; no Evidence body read、Evidence Store write、Agent execution、report publication、production scheduler、Gate G5 或 formal backtest promotion | AI/RE/SEC | 2026-07-30 |
| AEV-106 | SAL-P5-018 / Gate G5 | Gate G5 可信研究评审、可信报告链路、Agent 评测、预算缓存和安全边界验证记录 | [gate-g5-trusted-research-review.md](./gate-g5-trusted-research-review.md); [test_gate_g5_trusted_research_review.py](../tests/gates/test_gate_g5_trusted_research_review.py); [2026-07-30-gate-g5-trusted-research-review.md](./superpowers/plans/2026-07-30-gate-g5-trusted-research-review.md) | Red target `1 failed, 1 passed` with missing `docs/gate-g5-trusted-research-review.md`; Green focused target `2 passed`; related P5 suite `78 passed`; full pytest `497 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; live patched DSA worktree `--check-only` hit expected already-applied context conflict at `0004`; clean temp DSA worktree sequentially applied `0001..0007`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; executable contract verifies TrustedResearchReport canonical JSON authority、ResearchReportPage claim/citation/evidence/artifact expansion、Agent Golden 56-case thresholds (`citation_accuracy=1.0`, `unsupported_numeric_rate=0.0`, `schema_success_rate=1.0`, safety core passed)、Model Routing cache-hit zero-cost receipt reuse、Source Trust malicious instruction removal、AgentToolSecurity `shell.run` deny、GET-only report routes and no sender/Provider/LLM/Worker/Qlib/runtime promotion | TL/AI/RE/SEC | 2026-07-30 |
| AEV-107 | SAL-P6-001 | Auth/RBAC、desktop/standalone/team 身份模型、权限矩阵、OIDC claim mapping 和 API authorization catalog 验证记录 | [auth-rbac.md](./auth-rbac.md); [auth_rbac.py](../src/serenity_alpha_lab/application/auth_rbac.py); [test_auth_rbac.py](../tests/application/test_auth_rbac.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-30-auth-rbac.md](./superpowers/plans/2026-07-30-auth-rbac.md) | Red target failed with missing `serenity_alpha_lab.application.auth_rbac` (`1 error`); focused Green `5 passed`; Auth/RBAC architecture guard `1 passed`; related Auth/RBAC/API errors/architecture suite `41 passed`; full pytest `503 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers desktop local owner without OIDC/team setup、standalone local roles、team-mode data/run/config/admin separation、tenant/team mismatch deny、optional OIDC provider declaration and pre-validated claim mapping without network/JWT SDK、API authorization requirements for auth/config/user/quant/evidence/report/notification/audit surfaces and no FastAPI/Authlib/JWT/requests/Provider/LLM/Worker/Qlib/SQLAlchemy/DSA runtime import boundary; no object signed URL、Secret Manager、notification sender、production scheduler 或 formal backtest promotion | BE/SEC | 2026-07-30 |
| AEV-108 | SAL-P6-002 | Resource/Artifact 对象级授权、短时 signed URL、审计记录和 Worker least-privilege grant 验证记录 | [resource-artifact-authorization.md](./resource-artifact-authorization.md); [resource_authorization.py](../src/serenity_alpha_lab/application/resource_authorization.py); [test_resource_authorization.py](../tests/application/test_resource_authorization.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py) | Red target failed with missing `serenity_alpha_lab.application.resource_authorization` (`1 error`); architecture Red failed with missing module file (`1 failed`); focused Green `6 passed`; ResourceAuthorization architecture guard `1 passed`; related Resource/Auth/Artifact/Evidence/Backtest/Report/Architecture suite `62 passed`; full pytest `510 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers tenant/team/private owner deny、cross-tenant ID guessing deny、Artifact parent resource binding、short-lived HMAC URL tamper/duplicate-query/expiry verification、audit records without raw signatures/secrets、run-bound task-scoped Worker grant with explicit run/artifact ids and no FastAPI/Authlib/JWT/requests/Provider/LLM/Worker/Qlib/SQLAlchemy/DSA runtime import boundary; no Secret Manager、object storage adapter、notification sender、production scheduler 或 formal backtest promotion | BE/SEC | 2026-07-30 |
| AEV-109 | SAL-P6-003 | Secret Manager/OS Keychain 引用、配置 API safe diagnostics、轮换计划、配置审计和 runtime-free boundary 验证记录 | [secret-config-hardening.md](./secret-config-hardening.md); [config_profiles.py](../src/serenity_alpha_lab/application/config_profiles.py); [test_config_profiles.py](../tests/application/test_config_profiles.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-31-secret-config-hardening.md](./superpowers/plans/2026-07-31-secret-config-hardening.md) | Red target failed during collection with missing `ConfigAuditAction` from `serenity_alpha_lab.application.config_profiles` (`1 error`); focused Green `13 passed`; ConfigProfiles architecture guard `1 passed`; related Config/Auth/Resource/Architecture suite `57 passed`; full pytest `515 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; contract covers `env://`/`keychain://`/`secretmanager://` references、raw secret/query/fragment rejection、API diagnostics with configured/backend/last_four only for secret fields、dry-run rotation record with old/new reference hashes、deterministic audit id/hash and metadata redaction; no keyring/boto/hvac/cloud SDK/FastAPI/Authlib/JWT/requests/Provider/LLM/Worker/Qlib/SQLAlchemy/DSA runtime import boundary; no config route、`.env` write、database persistence、backup/restore、notification sender、production scheduler 或 formal backtest promotion | BE/SEC | 2026-07-31 |
| AEV-110 | SAL-P6-004 | URL/fetch policy、upload metadata scanning、trusted report HTML/link sanitizer、security headers 和 runtime-free boundary 验证记录 | [input-fetch-report-hardening.md](./input-fetch-report-hardening.md); [input_fetch_security.py](../src/serenity_alpha_lab/application/input_fetch_security.py); [report_delivery.py](../src/serenity_alpha_lab/application/report_delivery.py); [test_input_fetch_report_security.py](../tests/application/test_input_fetch_report_security.py); [test_report_delivery_ui.py](../tests/application/test_report_delivery_ui.py); [test_architecture_boundaries.py](../tests/architecture/test_architecture_boundaries.py); [2026-07-31-input-fetch-report-hardening.md](./superpowers/plans/2026-07-31-input-fetch-report-hardening.md) | Red target failed during collection with missing `serenity_alpha_lab.application.input_fetch_security` (`1 error`); focused Green `6 passed`; architecture/focused security `8 passed`; related SourceTrust/AgentToolSecurity/Renderer/Delivery/Architecture suite `53 passed`; full pytest `520 passed, 3 skipped`; compileall PASS; dependency lock guard PASS with `Resolved 298 packages`; immutable tag stayed `e8a9ca7742e8cb2498c8f491dd76d239b3064e1a`; `git diff --check` PASS; contract covers HTTPS/public-IP/host-allowlist URL metadata policy、redirect/size/content-type denial、upload filename/type/size/signature denial、report active HTML and unsafe link denial、source_link sanitization and CSP/security headers; no live fetch/DNS/browser/upload parser/AV/FastAPI/requests/httpx/Provider/LLM/Worker/Qlib/SQLAlchemy/DSA runtime import boundary; no notification sender、production scheduler、release packaging 或 formal backtest promotion | SEC/BE/FE | 2026-07-31 |
...
当前已完成 `SAL-P0-001` 至 `SAL-P0-013`、`SAL-P1-001` 至 `SAL-P1-016`、`SAL-P2-001` 至 `SAL-P2-020`、`SAL-P3-001` 至 `SAL-P3-017`、`SAL-P4-001` 至 `SAL-P4-022`、`SAL-P5-001` 至 `SAL-P5-018`、`SAL-P6-001` 至 `SAL-P6-004`，完成度为 110/129；最近阶段性任务为 `SAL-P6-004` 加固输入、抓取与报告渲染；最近可评审交付 checkpoint 为 `47fd9cf2 feat(P6): 加固输入抓取与报告渲染`，上一可评审交付 checkpoint 为 `984813fd feat(P6): 加固密钥与配置`。Gate G0、G1、G2、G3、G4、G5 已通过（均为 `GO with accepted risks`），G6 未通过；下一步为 `SAL-P6-005` 建立安全与供应链门禁，但本次不启动 `SAL-P6-005` 实现、`SAL-P6-006+`、真实 Provider/LLM、Worker loop、Qlib runtime、生产调度、通知 sender、release packaging、正式组合回测推广或其他 P6 后续任务；后续实现必须遵守 ADR-001/002、ADR-009、DEC-087 至 DEC-108 与 Gate G2/G3/G4/G5，不得从 Agent Stage Store、ModelInvocationPlanner、Prompt Registry、Citation Validator、AgentToolSecurityGuard、TrustedResearchReportRenderer、ResearchReportPagePresenter、NotificationOutboxStore、AgentEvaluationScorer、Quant Evidence Adapter、source trust、Technical/Intel/RiskPortfolio/Decision Agent adapter、EvidenceBundle、Auth/RBAC catalog、ResourceAuthorizationPolicy、SignedArtifactUrlIssuer、WorkerResourceGrant、ConfigProfiles secret/config contracts 或 InputFetchSecurity contracts 直接启动真实 Provider/LLM、Worker loop、Qlib runtime、生产调度、正式组合回测推广或未经批准的大规模 DSA 源码迁移。
