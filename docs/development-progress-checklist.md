...
- [ ] [READY] 用 3~5 证券、20~60 日手工样本覆盖关键规则
...
当前已完成 `SAL-P0-001` 至 `SAL-P0-013`、`SAL-P1-001` 至 `SAL-P1-016`、`SAL-P2-001` 至 `SAL-P2-020`、`SAL-P3-001` 至 `SAL-P3-017` 和 `SAL-P4-001` 至 `SAL-P4-018`，完成度为 84/129；最近阶段性任务为 `SAL-P4-018` 资源限制、取消和 checkpoint；最近实现 checkpoint 为 `7366eff7 feat(P4): 实现回测资源控制与 checkpoint`，上一可评审交付 checkpoint 为 `0badd06d feat(P4): 实现 BacktestRun 编排`；最新状态同步 checkpoint 将由本次状态同步提交生成，上一状态同步 checkpoint 为 `553771ea docs: 同步 SAL-P4-017 checkpoint hash`，最新状态同步 hash-anchor checkpoint 暂沿用 `6713a971 docs: 记录 SAL-P4-017 状态同步 hash`，最新最终锚点 checkpoint 暂沿用 `e616de21 docs: 固化 SAL-P4-017 hash-anchor checkpoint`，最新状态复核 checkpoint 暂沿用 `72df2fe2 docs: 复核 SAL-P4-017 最新开发状态与恢复提示`。Gate G0、G1、G2、G3 已通过（均为 `GO with accepted risks`），Gate G4 仍未通过。下一步优先执行 `SAL-P4-019` 回测金标与性质测试；后续实现必须遵守 ADR-001/002、ADR-009、Gate G1/G2/G3 和 P4 入口约束，不得跳过回测金标/正式 API 等任务直接启动 Quant Lab、Evidence Agent、真实 Provider/LLM 或 Worker loop，且不得把 Qlib internal backtest evidence、Dataset 转换或 legacy Signal Evaluation 命名为正式组合回测。
