...
| P5 Agent 与报告 | 13~16 | DOING | 6/18 | G5 | Evidence、引用、预算、可信报告 |
| P6 发布加固 | 16~18 | TODO | 0/23 | G6 | RC、稳定性、安全、发布与 Runbook |
| **合计** | **16~18 周** | **DOING** | **94/129** |  |  |
...
- [ ] [READY] 为 DSA Orchestrator 增加 stage_id、attempt 和 checkpoint
...
当前已完成 `SAL-P0-001` 至 `SAL-P0-013`、`SAL-P1-001` 至 `SAL-P1-016`、`SAL-P2-001` 至 `SAL-P2-020`、`SAL-P3-001` 至 `SAL-P3-017`、`SAL-P4-001` 至 `SAL-P4-022`、`SAL-P5-001` 至 `SAL-P5-006`，完成度为 94/129；最近阶段性任务为 `SAL-P5-006` Prompt 与输出 Schema Registry；最近可评审交付 checkpoint 为 `cccc1416 feat(P5): 实现 Prompt 与输出 Schema Registry`，上一可评审交付 checkpoint 为 `890ac789 feat(P5): 实现 Quant Evidence Adapter`，最近状态同步 checkpoint 为 `49a8cd23 docs: 同步 SAL-P5-006 checkpoint hash`，最新 hash-anchor checkpoint 为 `252189c3 docs: 记录 SAL-P5-006 状态同步 hash`，上一 hash-anchor checkpoint 为 `a724fdb8 docs: 固化 SAL-P5-004 hash-anchor checkpoint`，上一 Gate checkpoint 为 `1466c11c docs(P4): 通过 Gate G4 回测与风控评审`。Gate G0、G1、G2、G3、G4 已通过（均为 `GO with accepted risks`），G5 未通过；下一步优先执行 `SAL-P5-007` Agent Stage 持久化，暂不跳到 Agent 改造任务；后续实现必须遵守 ADR-001/002、ADR-009、DEC-087、DEC-088、DEC-089、DEC-090、DEC-091、DEC-092 与 Gate G2/G3/G4，不得从 Quant Evidence Adapter、source trust 或 EvidenceBundle 直接启动 Evidence Agent、真实 Provider/LLM、Worker loop、Qlib runtime、正式组合回测推广或未经批准的大规模 DSA 源码迁移，且不得把 Qlib internal evidence、Dataset 转换、Screen result、AlphaSift T+N evaluation 或 legacy Signal Evaluation 命名为正式组合回测。
