...
| P5 Agent 与报告 | 13~16 | DOING | 11/18 | G5 | Evidence、引用、预算、可信报告 |
| P6 发布加固 | 16~18 | TODO | 0/23 | G6 | RC、稳定性、安全、发布与 Runbook |
| **合计** | **16~18 周** | **DOING** | **99/129** |  |  |
...
- [ ] [READY] 在 DSA LiteLLM 上实现调用/Run/日三级预算
...
当前已完成 `SAL-P0-001` 至 `SAL-P0-013`、`SAL-P1-001` 至 `SAL-P1-016`、`SAL-P2-001` 至 `SAL-P2-020`、`SAL-P3-001` 至 `SAL-P3-017`、`SAL-P4-001` 至 `SAL-P4-022`、`SAL-P5-001` 至 `SAL-P5-011`，完成度为 99/129；最近阶段性任务为 `SAL-P5-011` 多空反证与最终综合；最近可评审交付 checkpoint 为 `50e6aa39 feat(P5): 实现多空反证与最终综合`，上一实现 checkpoint 为 `22ecff19 feat(P5): 改造 Risk Portfolio Agent`；最新状态同步 checkpoint 为 `e9e5ad69 docs: 同步 SAL-P5-011 checkpoint hash`，上一状态同步 checkpoint 为 `490590ba docs: 同步 SAL-P5-010 checkpoint hash`，最新状态同步 hash-anchor checkpoint 为 `c675fa0b docs: 记录 SAL-P5-011 状态同步 hash`，上一状态同步 hash-anchor checkpoint 为 `dbe74521 docs: 记录 SAL-P5-010 状态同步 hash`，最新最终锚点 checkpoint 为 `f6464426 docs: 固化 SAL-P5-010 状态复核 hash`，最新状态复核 checkpoint 为 `4f37b244 docs: 复核 SAL-P5-010 最新开发状态与恢复提示`，最新状态复核 hash-anchor checkpoint 为 `188d9b78 docs: 记录 SAL-P5-010 状态复核 hash`；上一状态同步 checkpoint 为 `7521d6d9 docs: 同步 SAL-P5-009 checkpoint hash`，上一状态同步 hash-anchor checkpoint 为 `c2da5fe8 docs: 记录 SAL-P5-009 状态同步 hash`，上一状态复核 checkpoint 为 `ec5208de docs: 复核 SAL-P5-009 最新开发状态与恢复提示`，上一可评审交付 checkpoint 为 `74701974 feat(P5): 改造 Technical Agent`，上一状态同步 checkpoint 为 `6b251300 docs: 复核 SAL-P5-008 最新状态与交接习惯`，上一 hash-anchor checkpoint 为 `8c978cd5 docs: 记录 SAL-P5-008 交接习惯复核 hash`，上一 Gate checkpoint 为 `1466c11c docs(P4): 通过 Gate G4 回测与风控评审`。Gate G0、G1、G2、G3、G4 已通过（均为 `GO with accepted risks`），G5 未通过；下一步优先执行 `SAL-P5-012` 模型路由、缓存与预算，暂不跳到 Citation Validator、报告渲染或 P5 后续任务；后续实现必须遵守 ADR-001/002、ADR-009、DEC-087、DEC-088、DEC-089、DEC-090、DEC-091、DEC-092、DEC-093、DEC-094、DEC-095、DEC-096、DEC-097 与 Gate G2/G3/G4，不得从 Agent Stage Store、Quant Evidence Adapter、source trust、Prompt Registry、Technical/Intel/RiskPortfolio/Decision Agent adapter 或 EvidenceBundle 直接启动真实 Provider/LLM、Worker loop、Qlib runtime、生产调度、报告生成、正式组合回测推广或未经批准的大规模 DSA 源码迁移，且不得把 Qlib internal evidence、Dataset 转换、Screen result、AlphaSift T+N evaluation 或 legacy Signal Evaluation 命名为正式组合回测。
