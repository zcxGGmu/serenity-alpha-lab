...
- [ ] [READY] 建立现金、持仓 Lot、应收应付、成交和估值账本
...
当前已完成 `SAL-P0-001` 至 `SAL-P0-013`、`SAL-P1-001` 至 `SAL-P1-016`、`SAL-P2-001` 至 `SAL-P2-020`、`SAL-P3-001` 至 `SAL-P3-017` 和 `SAL-P4-001` 至 `SAL-P4-008`，完成度为 74/129；最近阶段性任务为 `SAL-P4-008` 订单状态机；最近实现 checkpoint 为 `4927ba6f feat(P4): 实现订单状态机`，上一可评审交付 checkpoint 为 `6e81ae6f feat(P4): 实现 Qlib QuantEngine Adapter`；最新状态同步 checkpoint 为 `e58823a4 docs: 同步 SAL-P4-008 checkpoint hash`，最新状态复核 checkpoint 为 `dc88df2d docs: 复核 SAL-P4-007 最新开发状态与恢复提示`。Gate G0、G1、G2、G3 已通过（均为 `GO with accepted risks`），Gate G4 仍未通过。下一步优先执行 `SAL-P4-009` Portfolio Ledger；后续实现必须遵守 ADR-001/002、ADR-009、Gate G1/G2/G3 和 P4 入口约束，不得在 Ledger 任务外启动正式组合回测运行、Evidence Agent、真实 Provider/LLM、Risk/Quant Lab 或 Worker loop，且不得把 Qlib internal backtest evidence、Dataset 转换或 legacy Signal Evaluation 命名为正式组合回测。
